...
impression_category=music)，（**注：上面Memorization使用的是具体的app，而此处Generalization使用的仅仅是app的category**），但人工的特征工程通常是必需的。cross-product transformation的一个限制是，不能泛化到那些在训练数据上没有出现过的query-item特征对。
