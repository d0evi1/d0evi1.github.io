...
我们也会假设这样的一个随机策略（stochastic policy），**在actions上的分布**依赖于当前state以及关于agent参数的real valued vector $$\theta$$：
...
该方法的优点是：actions是deterministic，整个history会从单个参数样本上被生成。这种在samples-per-history上的reduction，会减小在gradient estimate中的variance。作为一个辅加好处，参数梯度（parameter gradient）会通过直接参数扰动（perturbations）进行估计，无需对任何派生物（derivatives）进行backpropagate，它会允许使用无微分的controllers。
...
注意，h是在条件独立于在给定$$\theta$$下的$$\rho$$，我们有：$$p(h, \theta \mid \rho) = p(h \mid \theta) p(\theta \mid \rho)$$，因此：$$\nabla_{\rho} log p(h,\theta \mid \rho) = \nabla_{\rho} log p(\theta \mid \rho)$$。将它代入等式(7)生成：
