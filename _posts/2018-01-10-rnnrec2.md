...
我们使用该数据来尝试拟合不同size的模型，特别的，我们会使用离散特征嵌入的模型，并采用拼接特征（concatenated）作为输入。该模型只有一个hidden layer，它使用ReLU activation，接着跟最后一个线性层。该模型使用tensorflow编码，使用MSE训练，并使用Adagrad optimizer，训练直到收敛。我们在训练数据和模型预测间使用Pearson correlation(R)来衡量并上报了模型的accuracy。我们使用Pearson相关系数，以便在数据的方差上有细微的不同可以认为是不变的。我们在训练数据上上报了accuracy，因为我们会测试这些模型结构是如何较好地拟合低秩模式（例如：..）。
