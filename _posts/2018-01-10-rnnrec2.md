...
为了理解神经推荐系统是如何利用拼接特征（concatenated features）的，我们研究了典型的这些网络的构建块。如上所述，神经网络（特别是前馈DNNs），通常会在一阶操作（first-order op）上构建。更准确的说，神经网络通常依赖于矩阵向量乘法（$$Wh$$），其中：W是一个学习得到的权重矩阵，h是一个input（可以是一个网络的input，也可以是前一layer的output）。在前馈网络中，FC layers可以以这种形式描述：
...
为了测试一阶神经网络是否可以建模低秩关系，**我们可以生成人工合成的低秩数据，并研究不同size的神经网络是如何拟合数据的**。确切的说，我们可以考虑一个m-mode的tensor（相当于：m个域），其中它每个维度(域)都是size N。对于$$mN$$的离散特征，我们会生成长度为r的随机向量$$u_i$$，它使用一个简单等式：
...
结果是，我们的数据是一个与它近似相同规模的rank为r的矩阵或者tensor（均值为0, 经验方差接近为1）。例如，m=3, 我们可以使用这些embeddings来表示形式为(i, j, t, $$\langle u_i, u_j, u_t \rangle$$)的events。
