...
另外，网络顶部用于分类的softmax layers是显式的bi-linear layers，它位于DNN产生的embedding与label classess的embeddings之间。该技术在多篇paper上被提及，包含DNN顶部的：user-item bi-linear layers[20,41,43,47]。
...
为了理解神经推荐系统是如何利用拼接特征（concatenated features）的，我们研究了一些典型的网络构建块。**如上所述，神经网络（特别是前馈DNNs），通常会在一阶操作（first-order op）上构建**。更准确的说，神经网络通常依赖于矩阵向量乘法（$$Wh$$），其中：W是一个学习得到的权重矩阵，h是一个input（可以是一个网络的input，也可以是前一layer的output）。在前馈网络中，FC layers可以以这种形式描述：
...
其中，g是一个element-wise操作（比如：一个sigmoid或ReLU），$$h_{(\tau-1)}$$是前一层的output，而$$W_{\tau}$$和$$b_{r}$$是学到的参数。我们将它认为是一个在$$h_{(\tau-1)}$$上的一阶单元（first-order cell），$$h_{(\tau-1)}$$是一个k维向量，不同的值会基于基于W的权重一起求和，而非相乘。

尽管神经网络可以使用layers来逼近任意函数，它们的核心计算与过去的CF的思想在结构上有很大不同。矩阵分解模型会采用通用形式$$u_i \cdot v_j$$，从不同类型的输入（users, items, time）产生模型学习低秩关系。这样的低秩模型在推荐系统中已经很成功，我们会问以下问题：**使用一阶单元的神经网络如何去建模低秩关系？**

## 4.1 建模低秩关系

为了测试一阶神经网络是否可以建模低秩关系，**我们可以生成人工合成的低秩数据，并研究不同size的神经网络是如何拟合数据的**。确切的说，我们可以考虑一个m-mode的tensor（相当于：m个域或元组），其中它每个维度(域)都是size N。对于$$mN$$的离散特征，我们会使用下面的一个简单等式来生成长度为r的随机向量$$u_i$$：

$$
u_i \sim N(0, \frac{1}{r^{(1/2m)}} I)
$$

...(2)

最后产生的结果数据是一个与（均值为0, 经验方差接近为1）近似相同的rank为r的matrix或者tensor。例如，对于m=3，我们可以使用这些embeddings（即：生成的matrix或tensor）来表示形式为(i, j, t, $$\langle u_i, u_j, u_t \rangle$$)的events。

我们尝试使用该数据来拟合不同size的模型。特别的，我们会考虑这样一个模型：它使用嵌入的离散特征和拼接特征（concatenated）一起作为输入。该模型只有一个hidden layer，它使用ReLU activation，接着跟最后一个线性层。该模型使用tensorflow编写，使用MSE训练，并使用Adagrad optimizer训练直到收敛。我们在训练数据和模型预测间使用Pearson correlation(R)来衡量并上报了模型的accuracy。我们使用Pearson相关系数，以便在数据的方差上有细微的不同可以认为是不变的。我们在训练数据上上报了accuracy，因为我们会测试这些模型结构对于拟合低秩模式的好坏程度（例如：是否可以从它进行泛化）。

为了建模低秩关系，我们希望看到，模型逼近单一乘法（它可以表示变量之间的交叉）的好坏程度。所有数据均使用N=100生成。对于m=2，我们会检查为让两标量相乘hidden layer需要的大小；对于m=3，我们会检查为让三个标量相乘hidden layer需要的大小。我们会使用$$ r \in \lbrace 1,2 \rbrace$$来看到，模型size随所需乘法数是如何增长。我们会嵌入每个离散特征作为一个20维向量，比r更大（但我们发现：模型的accuracy会与该size独立）。我们测试了hidden layers  $$\in \lbrace  1,2,5,10,20,30,50 \rbrace$$。
