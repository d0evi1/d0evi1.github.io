...
我们使用该数据来尝试拟合不同size的模型，特别的，我们会使用离散特征嵌入的模型，并采用拼接特征（concatenated）作为输入。该模型只有一个hidden layer，它使用ReLU activation，接着跟最后一个线性层。该模型使用tensorflow编写，使用MSE训练，并使用Adagrad optimizer训练直到收敛。我们在训练数据和模型预测间使用Pearson correlation(R)来衡量并上报了模型的accuracy。我们使用Pearson相关系数，以便在数据的方差上有细微的不同可以认为是不变的。我们在训练数据上上报了accuracy，因为我们会测试这些模型结构是如何较好地拟合低秩模式（例如：是否可以从它进行泛化）。

为了建模低秩关系，我们希望看到，模型是如何很好地逼近单一乘法（它可以表示变量之间的交叉）。所有数据使用N=100生成。有了m=2, 我们会检查为让两标量相乘hidden layer需要多大， m=3,我们会检查为让三个标量相乘hidden layer需要多大。我们会使用$$ r \in \lbrace 1,2 \rbrace$$来看到，模型size随所需乘法数是如何增长。我们会嵌入每个离散特征作为一个20维向量，比r更大（但我们发现：模型的accuracy会与该size独立）。我们测试了hidden layers  $$\in \lbrace  1,2,5,10,20,30,50 \rbrace$$。
