...
例如，Netflix Prize setting中，使用tuples $$e \equiv (i,j,R)$$，表示user i对电影j有个评分R。我们加上context信息(比如：时间和设备)：$$e \equiv (i,j,t,d)$$，表示用户i观看了视频j，在时间点t设备类型d上。注意，每个值即可以是离散化的类别型变量(比如在其中有N个用户，其中$$i \in I$$)，也可以是连续型（比如：t是一个unix timestamp） 。连续型变量在预处理阶段被离散化很常见，比如：将t转换成event发生的天数。
