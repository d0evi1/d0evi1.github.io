...
在本节中，我们会对multi-domain retrieval任务进行公式化定义。**Multi-domain retrieval task的目标是：从一个非常大的corpus中，为multiple domains检索high-quality items**。更特别的，假设：U和V分别表示user set 和item set。online multi-domain retrieval任务可以被公式化：

$$
S_{u,d} = \underset{{v \in V}}{arg \ Topk} f_{\theta}(u, d) 
$$
