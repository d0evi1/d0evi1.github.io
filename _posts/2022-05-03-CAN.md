...
该过程的可视化如图3左侧所示。$$P_{feed}$$接着被feed到micro-MLP，特征交叉可以通过每个layer的output的concatentation来实现：
