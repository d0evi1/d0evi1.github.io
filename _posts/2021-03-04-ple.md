...
- $$w_{VTR}, w_{VCR}, w_{SHR}, w_{CMR}$$：是通过在线实验搜索优化的超参数，用来最大化在线指标。（注：SHR（SHare Rate：分享率）、CMR（Comment Rate：评论率））
...
- VCR预测是一个**回归任务（regression task）**，它使用MSE loss来预测每次播放完成度。
- VTR预测是一个**二分类任务**，它使用cross-entropy loss来预测一个有效播（valid view）的概率，它被定义成：**超过一定观看时间阈值的一次播放行为**。

在VCR和VTR间的相关模式（correlation pattern）很复杂。

- 首先，VTR的label是一个关于播放动作和VCR的组合因子，只有watch time超过阈值的一个播放动作会被看成是一个**有效观看（view）**。
- 第二，播放动作的分布也复杂，因为**在wifi下来自自动播（auto-play）场景的样本会具有较高的平均播放率，而来自显式点击场景（没有auto-play）的其它样本会具有更低的播放率**。

由于复杂和强样本依赖的相关性，当联合建模VCR和VTR会观察到一个seesaw效应。

## 3.2 在MTL中的Seesaw效应

为了更好地理解seesaw效应，我们会使用单任务模型和SOTA MTL模型来执行实验分析，在复杂相关的VCR和VTR任务组上。除了硬参数共享、cross-stitch、sluice network、mmoe外，我们也评估了两个独创的结构：非对称共享（asymmetric sharing）和定制共享（customized sharing）。

- **非对称共享（asymmetric sharing）**：是一种新的共享机制，用来捕获在任务间的非对称关系。根据图1b，bottom layers会在任务间的非对称共享，具体某个任务的表示需要共享依赖于任务间的关系。**公共融合操作（fusion）（比如：concatenation、sum-pooling、average-pooling）可以用来组合不同任务的bottom layers的outputs**。
- **定制化共享（Customized Sharing）**：图1c会显式地将共享参数和task-specific参数进行分离，以避免内在冲突和negative transfer。对比起single-task模型，定制化共享会添加一个shared bottom layer来抽取共享信息，并将shared bottom layer与task-specific layer的concatenation后feed给相应任务的tower layer。
...
sharing可以达到在VTR上的极大提升，但在VCR上出现极大降低，这与sluice network类似。由于shared layers和task-specific layers的显式分隔，定制化共享可以在单任务模型上提升VCR，而在VTR上只有轻微的损耗。MMOE则会在两个任务上同时对单任务进行提升，但VCR的提升只有：+0.0001. 尽管这些模型会在这两个任务上具有不同的学习效率（learning efficiency），我们可以很明确地观察到seesaw效应：**一个任务的提升会导致其它任务的效果退化**，因为没有一个baseline MTL模型依完全落在第二象限。在公开的benchmark datasets上的具有SOTA模型的实验，也会具有明显的seesaw效应。细节会在第5.2节中提供。
...
如前所示，VCR和VTR间的相关模式是很复杂并且样本依赖（sample depedent）的。特别的，VCR和CTR间存在一些**偏序关系(partially ordered relations)**，不同样本表现出不同的相关度。因而：

- cross-stitch和sluice network会为所有样本**使用相同的static weights**来共享表示，不能捕获样本依赖，存在seesaw效应。
...
除了VCR和VTR外，在工业界推荐应用中有许多复杂相关任务，因为人类行为经常微妙且复杂，例如：在在线广告和电商平台中的CTR预测和CVR预测。因此，一个强大的网络需要考虑在**experts间的差异（differentiation）和交叉（interactions）**，这对于消除由复杂任务相关性带来的seesaw效应来说很重要。
...
- 第二，多层的experts和gating networks会被引入来对多个抽象表示（abstract representations）进行融合。
- 最后，它会采用一个新的渐进分离路由（progressive separation routing）来建模在专家间的交叉，并达到在复杂相关任务间更高效的知识迁移。
...
受定制化共享的启发，该方式通过显式分离shared  layers和task-specific layers，达到了与single-task模型相似的效果。如图4所示，在底部有一些**experts模块**，在顶部有一些**task-specific tower networks**上。

- 专家模块：每个由多个称为experts的子网络（sub-networks）构成，在**每个模块中的experts的数目是一个可调的超参数**
- tower network：一个multi-layer network，宽和高是超参数

特别的，在CGC中的shared experts负责学习共享模式（shared patterns），而对于specific tasks的模式会由task-specific experts来抽取。每个tower network会从shared experts和它自己的task-specific experts中吸收知识，这意味着shared experts的参数会被所有任务影响，而task-specific experts的参数具受相应specific task的影响。
...
- x是输入表征
- k表示task k
- $$w^k(x)$$是一个weighting函数，通过线性变换和一个Softmax layer来计算**task k**的weight vector：
...
- $$W_g^k \in R^{(m_k + m_s) \times d}$$是参数矩阵，$$m_s$$和$$m_k$$分别是shared experts以及第k个specific experts的数目，d是input representation的维度。
...
$$
L_k(\theta_k, \theta_s) = \frac{1}{\sum_i \sigma_k^i} \sum\limits_i \sigma_k^i loss_k(\widehat{y}_k^i (\theta_k, \theta_s), y_k^i))
$$
...
- $$loss_k$$是任务k基于prediction $$\widehat{y}_k^i$$、以及ground truth $$y_k^i$$的样本i的的loss
- **$$\sigma_k^i \in \lbrace 0,  1 \rbrace$$表示的是：样本i是否位于task k的样本空间**

第二个问题是：**一个MTL模型的效果对于在训练过程中loss weight的选择是否敏感【9】，因为它决定了在joint loss上每个任务的相对重要性**。实际上，这会观察到：每个任务在不同的训练过程会具有不同的重要性。因此，我们会为每个task考虑loss weight作为一个**动态权重（dynamic weight）**，而非一个static权重。首先，我们会为task k设置一个初始的loss weight $$w_{k,0}$$，接着在每个step后基于更新率（updating ratio） $$\gamma_k$$更新它的loss weight：
...
图7 合成数据上的多标签学习（MTL）增益

## 5.2 public datasets上的评估

在本小节中，我们针对公共基准数据集进行了实验，以进一步评估PLE在推荐场景之外的有效性。

### 5.2.1 数据集

合成数据是根据[13]中的数据合成流程生成的，以控制任务相关性。由于[13]中没有提供数据合成的超参数，我们随机采样αi和βi，遵循标准正态分布，并设置c=1、m=10、d=512以实现可重复性。对于每个关系，生成了包含两个连续标签的140万个样本。

• 人口普查收入数据集[5]包含来自1994年人口普查数据库的299,285个样本和40个特征。为了与基线模型进行公平比较，我们考虑使用与[13]相同的任务组。具体来说，任务1旨在预测收入是否超过50K，任务2旨在预测这个人的婚姻状况是否从未结婚。

阿里-CCP数据集1是一个包含8400万个样本的公开数据集，这些样本是从淘宝推荐系统中提取出来的。CTR（点击率）和CVR（转化率）是数据集中模拟点击和购买行为的两个任务。

### 5.2.2 实验设置。

人口普查收入数据集的设置与[13]中的相同。对于合成数据和Ali-CCP数据集，我们采用三层MLP网络，每个任务都采用RELU激活函数和隐藏层大小分别为[256, 128, 64]。超参数的调整与第5.1节中的实验类似。

### 5.2.3 实验结果。

图7中显示的合成数据实验结果表明，硬参数共享和混合任务多模态学习（MMOE）有时会遭受跷跷板现象，即两个任务之间的平衡被打破。相反，混合任务多模态推理（PLE）在不同相关性下始终在两个任务上表现最佳，并且平均而言，与MMOE相比，其多任务学习时间（MTL）增益提高了87.2%。如表5所示，在阿里混元CP和人口收入数据集上的结果显示，PLE消除了跷跷板现象，并且在两个任务上都持续优于单任务模型和MMOE。

结合之前针对工业数据集的实验和在线A/B测试，PLE在不同任务相关模式和不同应用场景下，在提升多任务学习（MTL）效率和性能方面表现出稳定的总体优势。

## 5.3 Expert使用分析

为了探索experts是如何通过不同gates间进行聚合的，我们在工业界dataset上的VTR/VCR task group上，研究了所有gate-based models的expert utilization。出于简洁性和公平对比，我们会考虑**将每个expert看成是一个single-layer network**，在CGC和PLE的每个expert module上保持一个expert，而在MMOE和ML-MMOE的**每个layer则会保持三个experts**。图8展示了在所有testing data上每个gate使用的experts的权重分布，其中：bars的高度以及垂直short lines分别表示weights的**均值和标准差**。它表明：**VTR和VCR在CGC中会使用极不同的weights来组合experts，而在MMOE中则使用非常相似的weights**，这表明：CGC的良好设计结构可以帮助达到在不同experts间更好的区分度。另外，在MMOE和ML-MMOE中所有experts都有非零权重，这进一步表明：对于MMOE和ML-MMOE来说，在没有先验知识的情况下，很难去收敛CGC和PLE的结构，尽管存在理论可能性。对比起CGC，在PLE中的shard experts对tower networks的input具有更大的影响，特别是在VTR任务上。实际上，PLE的效果要好于CGC，这表明在更高level上共享更深的representations的价值。换句话说，需要在任务间共享的更深语义表示，因此 一个progressive separation routing可以提供一个更好的joint routing和learning scheme。
...
在本文中，我们提出了一种新颖的混合迁移学习（MTL）模型，称为渐进分层提取（PLE），它明确地分离了任务共享参数和特定任务参数，并引入了一种创新的渐进式路由方式来避免负向转移和跷跷板现象，从而实现更高效的信息共享和联合表示学习。在工业数据集和公共基准数据集上的离线和在线实验结果表明，PLE相较于现有的SOTA MTL模型有显著且一致的改进。探索层次化任务组相关性将是未来工作的重点。
