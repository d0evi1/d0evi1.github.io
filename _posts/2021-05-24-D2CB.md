...
选中的super arm S的diversity可以通过intra-list distance metric（Zhang&Hurley 2008）进行measure。一旦一个多性性的super arm S根据一个policy $$\pi$$被选中并展示给用户，该用户在展示items的engagements（例如：在items上的点击）会用来作为推荐agent的rewards，用来最优化它的推荐policy。通过与用户交互，推荐agent的目标是调整它的super arm选择策略来最大化它的随时间的累积回报（cumulative reward over time）。
