...
- 1）**LogRank**：在该方法中，我们会定义每个arm $$a_i$$的quality score为：$$r_i = 1/(1+ exp(-\bar{u}x_i^T))$$，其中：$$\bar{u}$$是从训练数据中学到user embeddings的平均。接着，$$\| S_t \|$$个具有最高quality scores的arms会被选中作为一个super arm $$S_t$$来进行第t次trail的推荐
