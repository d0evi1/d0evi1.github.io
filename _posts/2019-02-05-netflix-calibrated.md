...
在第一个以及最极端情况下，假设：我们只知道用户在genres上的偏好，但我们没有在每个genre内的单个电影的额外信息。在缺少任何额外信息的情况下，该问题与机器学习中的不平衡分类问题（imbalanced classification）相类似：通过预测majority class的label，总是会获得最好的预测accuray。**在一个二分类问题中，已知有70%的数据点的label为+1的，而剩余30%数据点的label为-1，在缺少其它额外信息的情况下，为所有数据点预测label为+1总是最好的——因为会有70%的数据点是正确的**。相反的，如果我们随机使用概率70%和30%（出现在数据中的概率）来预测label +1和label -1，我们期望的预测labels只有$$0.7 \cdot 70% + 0.3 \cdot 30% = 58%$$的数据点会是正确的。
