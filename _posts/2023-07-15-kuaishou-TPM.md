...
- 首先，尽管观看时长是个连续值，它也是一个序数变量（ordinal variable），它的值的相对序会影响着用户体验上的差异。因此，这种序数关系（ordinal relations）会在观看时长预估s中被影响。
- 第二，在视频观看行为间的条件依赖，可以在模型中被捕获。例如，在用户完成观看整个视频之前，它必须已经看了视频的一半。
- 第三，使用点估计（point estimation）来建模观看时长会忽略掉以下事实：模型会给出具有高度不确定的结果，这会造成badcase
- 第四，真实推荐系统会有偏差放大（bias amplifications），因而需要进行无偏估计

如何设计一个框架来解决上述4个问题仍是未被探索充分的。因此，我们提出TPM（Tree-based Progressive regression Model）来进行观看时长预估。特别的，观看时长的序（ordinal ranks）会被引入到TPM中，该问题被解耦成一系列的条件依赖分类任务，它们会以树结构进行组织。期望观看时长可以通过遍历该tree来生成，观看时长预估的变种会被显式引入到目标函数中作为不确定性（uncertianty）的一种measurement。再者，我们演示了后门调整（backdoor adjustment）可以被无缝混合在TPM中，它可以缓解偏差放大（bias amplifications）问题。
...
首先，观看时长预估本质上是一个regression问题，但在观看时长预估s间的顺序关系（ordinal relation）是在推荐中很重要。
...
我们首先判断：$𝑡(𝑥) ≤ 𝛾_0$ 还是反之。 如果$𝑡(𝑥) ≤ 𝛾_0$，则预测的序数等级是$𝛾_0$；否则，我们继续判断：$𝑡(𝑥) ≤ 𝛾_1$，还是反之。如果是真的，那么等级是$𝛾_1$，否则这个过程继续。这个过程一直进行，直到最终找到区间。

如果我们进行**二分搜索**，那么搜索过程就变成了：我们首先决定$𝑡(𝑥) \leq 𝛾_𝑚/2$：如果是真的，那么𝑡(𝑥)属于集合$\lbrace 𝛾_0, 𝛾_1, \cdots, 𝛾_𝑚/2 \rbrace$；否则它属于$\lbrace 𝛾_𝑚/2+1, \cdots, 𝛾_𝑚 \rbrace$的等级。这个过程继续，搜索空间缩小到某个等级。
...
因此，我们提出了一个基于树的渐进模型（TPM）用于观看时间预测。模型由问题分解的树T和相应的分类模型$\lbrace M_𝑖，𝑖 \in \lbrace 0, 1, ..., |𝑁_T| − |𝐿_T| \rbrace \rbrace$组成，其中：

- $𝑁_T$是T中的节点集合，
- $𝐿_T$是T中的叶节点集合（见图2为概览）

TPM中的树由一组节点组成：$T(X) = \lbrace 𝑁_T ∪ 𝐿_T \rbrace$。

每个非根节点代表一个由连续序数等级组成的区间，即$𝑛_𝑖：[𝛾_{𝑠_𝑖},𝛾_{𝑒_𝑖}]，𝑒_𝑖−𝑠_𝑖 > 1$。不失一般性，根节点假定为全空间：$𝑇 ∈ [𝛾_0,𝛾_𝑚]$。每个叶节点被分配一个特定的序数等级，即$𝑙_𝑖：[𝛾_𝑖, 𝛾_{𝑖+1}]$。并且父节点的子空间是其子节点的子空间的并集。从根到叶$𝑙_𝑘$的路径表示为有序节点集：$𝜙_{𝑙_𝑘} = \lbrace \hat{𝑛}_{𝜙_{𝑙_𝑘}(0)}, \cdots, \hat{𝑛}_{𝜙_{𝑙_𝑘}(𝑑(𝑙_𝑘))} \rbrace$, 其中：

- $\hat{𝑛}_{𝜙_{𝑙_𝑘}(𝑖)}$：是路径$𝜙_{𝑙_𝑘}$上的第𝑖层的节点
- $𝑑(𝑙_𝑘)$：是叶节点$𝑙_𝑘$的深度。
...
- $ 𝑝(𝑇 \in \hat{𝑛}_{𝜙_{𝑙_𝑘}(𝑖)}|𝑋, T,𝑇 \in \hat{𝑛}_{𝜙_{𝑙_𝑘(𝑖 − 1))}}$ 由分配给节点$𝑛ˆ𝜙𝑙𝑘(𝑖 − 1)$的分类器$M𝑛ˆ𝜙𝑙𝑘(𝑖−1)$参数化。

基于推导，给定树T的观看时间的期望可以如下计算：

$$
𝐸(𝑇 |𝑋, T) = ∑𝑙_𝑘 \in 𝐿_T 𝐸(𝑇 |𝑇 ∈ 𝑙𝑘, 𝑋, T)𝑝(𝑇 ∈ 𝑙𝑘|𝑋, T) (2)
$$

注意，期望涉及项$𝐸(𝑇 |𝑇 ∈ 𝑙_𝑘, 𝑋, T)$，这可以通过任何预测模型估计。在本文中，我们采用简单的方法进行估计：

$$
𝐸(𝑇 |𝑇 ∈ 𝑙𝑘, 𝑋, T) = (𝛾𝑙𝑘 + 𝛾𝑙𝑘+1)/2
$$

通过构建多个树进行观看时间估计，可以通过将树的分布作为先验来计算观看时间的期望：

$$
𝐸(𝑇 |𝑋) = ∑T 𝐸(𝑇 |𝑋, T)𝑝(T |𝑋) = ∑T 𝐸(𝑇 |𝑋, T)𝑝(T) (3)
$$
...
$$ \text{Var}(𝑇 |𝑋, T) = \mathbb{E}(𝑇^2 |𝑋, T) - \mathbb{E}(𝑇 |𝑋, T)^2 \quad (4) $$
请注意，𝑝(𝑇 ∈ 𝑙𝑘|𝑋, T)可以通过方程1计算，方差可以在假设𝑇 ∼ 𝑝(𝑇 ∈ 𝑙𝑘|𝑋, T)，∀𝑙𝑘 ∈ 𝐿T的情况下轻松计算。
