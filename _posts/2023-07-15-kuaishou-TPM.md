...
$ p(T \in \hat{𝑛}_{𝜙_{𝑙_𝑘}}(𝑖) \mid 𝑋,T, T \in \hat{𝑛}_{𝜙_{𝑙_𝑘}}(𝑖 − 1)) $ ：通过分配给节点$ \hat{n}_{𝜙_{l_k}(i− 1)} $的分类器$ M_{ \hat{𝑛}_{𝜙_{𝑙_𝑘}(𝑖−1)}}$来进行参数化。
...
通过构建多个树进行观看时间估计，可以通过**将树的分布作为先验来计算观看时间的期望**：
