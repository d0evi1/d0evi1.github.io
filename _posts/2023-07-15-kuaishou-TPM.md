...
每个非根节点代表一个由连续序数等级组成的区间，即$𝑛_𝑖：[𝛾_{𝑠_𝑖},𝛾_{𝑒_𝑖}]，𝑒_𝑖−𝑠_𝑖 > 1$。不失一般性，根节点假定为全空间：$𝑇 ∈ [𝛾_0,𝛾_𝑚]$。每个叶节点被分配一个特定的序数等级，即$𝑙_𝑖：[𝛾_𝑖, 𝛾_{𝑖+1}]$。并且父节点的子空间是其子节点的子空间的并集。从根到叶$𝑙_𝑘$的路径表示为有序节点集：
...
- $ 𝑝(𝑇 \in \hat{𝑛}_{𝜙_{𝑙_𝑘}(𝑖)}|𝑋, T,𝑇 \in \hat{𝑛}_{𝜙_{𝑙_𝑘(𝑖 − 1))}}$ 由分配给节点$ \hat{𝑛}_{𝜙_{𝑙_𝑘}(𝑖 − 1)}$的分类器$ M \hat{𝑛}_{𝜙_{𝑙_𝑘}}(𝑖−1)$参数化。

基于推导，给定树T的观看时间的期望可以如下计算：

$$
𝐸(𝑇 |𝑋, T) = ∑ 𝑙_𝑘 \in 𝐿_T 𝐸(𝑇 |𝑇 ∈ 𝑙_𝑘, 𝑋, T)𝑝(𝑇 ∈ 𝑙_𝑘|𝑋, T) (2)
$$

注意，期望涉及项$𝐸(𝑇 |𝑇 ∈ 𝑙_𝑘, 𝑋, T)$，这可以通过任何预测模型估计。在本文中，我们采用简单的方法进行估计：

$$
𝐸(𝑇 |𝑇 ∈ 𝑙_𝑘, 𝑋, T) = (𝛾_{𝑙_𝑘} + 𝛾𝑙𝑘+1)/2
$$
