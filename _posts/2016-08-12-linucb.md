...
算法1计算了矩阵的逆，$$D_a^T D_a + I_d $$（或者：$$D_a^T D_a$$），其中$$D_a$$是design matrix，它的行对应于训练数据中的特征。所有arms的这些矩阵具有固定维度d x d，可以高效地进行增量更新。另外，它们的逆（ inverses）可以通过算法1的disjoint参数进行轻易计算：在等式(3)中的解$$\hat{\theta}_a$$不会被其它arms的数据数据所影响，因为计算是独立的。我们现在考虑使用hybrid模型的case。
