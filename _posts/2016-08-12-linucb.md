...
我们提出的policy评估器如算法3所示。**该方法会将输入一个policy $$\pi$$以及一个基于该评估之上的关于“good”事件（event）T的期望值**。我们接着沿着日志事件的流一个接一个地前行。给定当前历史$$h_{t-1}$$，有：

- 如果发生策略$$\pi$$选择了与logging policy所选arm的相同的arm a，接着event仍被添加到历史中，total payoff $$R_t$$会被更新。
- 否则，如果policy $$\pi$$选择一个与logging policy所选arm所不同的arm，那么event会被完全忽略，该算法会处理下一event，在该state上无需任何变化。
