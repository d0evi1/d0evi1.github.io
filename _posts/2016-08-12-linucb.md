...
该模型称为disjoint的原因是：**不同arms间的参数不共享(每个arm各有一组权重，与m维特征存在加权关系得到期望payoff)**。假设$$D_a$$是在实验t上一个m x d维的设计矩阵(design matrix)，它的行对应于m个训练输入（例如：对于文章a，之前已经观察到的m个contexts），$$b_a \in R^m$$是相应的响应向量(response vector) （例如：相应的m个 点击/未点击 user feedback）。我们将岭回归（ridge regression）应用到训练数据$$(D_a, c_a)$$上，给定了系数的一个估计（即伪逆）：
...
<img src="http://pic.yupoo.com/wangdren23_v/61fc4d2b/13c2e1fd.jpeg" width=“320">
...
<img src="http://pic.yupoo.com/wangdren23_v/599417e1/35b0aade.jpeg" width=“320">
...
<img src="http://pic.yupoo.com/wangdren23_v/6d81dc28/67a3d1c1.jpeg" width=“320">
