...
本节中，提出的算法会对streaming documents和在它内的words进行联欢合建模，其中documents和words学到的向量表示会在一个共享的、低维的embedding空间内。该方法受[14]的启发。然而，与[13]的不相似之处是，我们会利用steaming documents中的temporal neighborhood。特别的，我们会建模一个特定文档的概率，并考虑时序上更接近（temporally-close）的文档，另外还会考虑文档内容。
