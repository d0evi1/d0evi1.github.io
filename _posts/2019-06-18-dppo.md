...
该期望会对应于由policy $$\pi_{\theta}$$和系统动态性(dynamics)：

$$
p(s_{t+1} \mid s_t, a_t): \rho_{\theta}(\tau) = p(s_0) \pi(a_0 \mid s_0) p(s_1, s_0, a_0) ...
$$

两者所联合生成的trajectories$$\tau=(s_0,a_0,s_1,\cdots)$$的分布。

$$\theta$$对应的目标函数的梯度是：

$$\nabla_{\theta} J = E_{\theta} [\sum_t \nabla_{\theta} log \pi_{\theta}(a_t \mid s_t)(R_t - b_t)]$$
