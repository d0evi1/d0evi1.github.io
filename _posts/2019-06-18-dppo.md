...
Policy gradient estimates可以有很高的variance，算法对于超参数的设置很敏感。有许多方法可以使得policy gradient算法更健壮。一种有效的measure是：采用一个trust region constraint，可以限制变更policy的任意更新量(amount)。采用该思想的一种流行算法是：TRPO（trust region policy optimization）。在每个迭代中，给定当前参数$$\theta_{old}$$，TRPO会收集一个（相对较大）的batch数据，并优化surrogate loss：
