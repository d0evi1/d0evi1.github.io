...
**prod2vec-cluster**：为了能做出更多样化的推荐，我们考虑将相似商品分组成聚类，从与之前购买商品相关的聚类中推荐商品。我们应用K-means聚类算法来在hadoop FS上实现，将商品基于cosine相似度进行聚类。我们假设：从任意第C个聚类中的购买了一个商品，在从聚类$$c_i$$上的购买符合一个正态分布$$M_u(\theta_{i1}, \theta_{i2}, ..., \theta_{iC})$$，其中$$\theta_{ij}$$是从聚类$$c_i$$中进行一次购买后、接着从聚类$$c_j$$中进行一次购买的概率。为了估计参数$$\theta_{ij}$$，对于每个i和j，我们采用一个最大似然方法：
