...
\$ \oplus \$为串联操作符(concatenation operator)。\$ x_{i:i+j} \$ 表示 \$ x_i \$至此\$ x_{i+j} \$的串联。卷积（convolution）操作符涉及到一个filter \$ w \in R^{hk} \$，它可以应用于一个含h个词(word)的窗口(window)，来生成一个新的feature。例如，可以由一个词窗口\$ x_{i:i+h-1}\$来生成一个特征\$c_i\$：
...
这里 \$ b \in R^{n-h+1} \$是一个bias项，f是一个非线性函数（例如：假设函数tangent）。在filter应用在每个可能的句子中的词窗口：\${x_{1:h}, x_{2:h+1},...,x_{n-h+1:n}}\$来生成一个特征图（feature map）。
...
其中\$ c \in h^{n-h+1}\$，我们接着在该feature map上应用一个max-over-time pooling操作，并采用最大值\$hat{c} = max{c} \$作为该指定filter相应的特征。该思路用来捕获最重要的特征--它带有最大值--对于每个feature map。该pooling scheme天然就可以处理不同的句子长度。

我们接着描述该过程，通过从一个filter上抽取一个feature。该模型使用多个filters(具有不同的窗口size)来获得多个feature。这些特征构成了倒数第二层（penultimate layer），并被传到一个fully connected softmaxy layer，它的输出为在label上的概率分布。

在一个模型变种中，我们试验了：具有两个词向量的通道（channels）——一个保持static throughout training，另一个通过backpropagation进行 fine-tuned。在多通道的架构上，如图1所示，每个filter被应用于多个channel。被添加的结果用来计算方程(2)式中的ci。该模型和单个channel的架构相等。
...
其中\$ \circ \$是element-wise乘法操作，\$ r \in R^{m}\$是一个关于Bernoulli随机变量的'masking'向量，它具有概率p的部分为1。梯度通过后向传播，只通过unmasked的单元。在测试时，学到的weight向量通过p进行归一化，例如：\$ \hat{w} = pw \$，其中\$ hat{w} \$被用来（没有dropout）对未见过的句子（unseen sentences）进行打分。我们又额外增加权重向量的l2-norms约束，通过对w进行rescaling，使得：\$ \|\|\|w\|\|\|_{2}=s \$，在经历一个梯度下降的step后，将永远\$ \|\|\|w\|\|\|_{2}>s \$。
...
- CNN-non-static: 与上面的方法相似，但pre-trained vectors对于每个任务都是fine-tuned。
- CNN-multichannel: 模型具有两个词向量集合。每个向量集都看成是一个'channel'，每个filter都会作用于两个channel，但梯度的后向传播只通过其中一个channel进行。这里模型可以fine-tune 一个向量集，让另一个保持static。两个channel都通过word2vec进行初始化
