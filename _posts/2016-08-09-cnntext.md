...
\$ \oplus \$为串联操作符。\$ x_{i:i+j} \$ 表示 \$ x_i \$至此\$ x_{i+j} \$的串联。卷积（convolution）操作符涉及到一个filter \$ w \in R^{hk} \$，它可以应用于一个含h个词(word)的窗口(window)，来生成一个新的feature。例如，可以由一个词窗口\$ x_{i:i+h-1}\$来生成一个特征\$c_i\$：
