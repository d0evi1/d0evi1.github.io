...
其中\$ c \in R^{n-h+1}\$，我们接着在该feature map上应用一个max-over-time pooling操作，并采用最大值\$ \hat{c} = max \\{ c \\} \$作为该指定filter相应的特征。该思路用来捕获最重要的特征--它带有最大值--对于每个feature map。该pooling scheme天然就可以处理不同的句子长度。
