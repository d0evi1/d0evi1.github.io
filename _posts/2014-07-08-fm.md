...
在监督学习领域，假设存在一个训练数据集：$$ D=\lbrace (x^{(1)},y^{(1)}), (x^{(2)},y^{(2)}), ... \rbrace$$，目标函数y给定的样本。我们也研究了排序任务，其中函数y的目标 T=R 可以被用于得分特征向量（score feature vectors）x，可以根据score进行排序。得分函数（score functions）可以通过pairwise的训练数据进行学习得到，其中特征tuple $$(x^{(A)}, x^{(B)}) \in D $$意味着$$ x^{(A)} $$的排序比$$x^{(B)} $$更高。由于pairwise的排序关系是斜对称（antisymmetric），只使用正例进行训练就足够了。
