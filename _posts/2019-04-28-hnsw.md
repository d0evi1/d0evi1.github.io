...
生成这样一个分层结构(layered structure)的一种方式是：**通过引入layers，使用不同length scales来显式设置links**。对于每一个element，我们会选择一个整数level l：它定义了该element所属layer的最大layer。对于在一个layer中的所有elements，会增量构建一个邻近图（proximity graph）（例如：graph只包含"short" links，它近似于Delaunay graph）。如果我们设置l的指数衰减概率（例如：根据一个几何分布），我们会获得在该结构中layers的期望数目的一个对数扩展。该search过程是一个迭代式greedy search，它从top layer开始，在zero layer完成。

**当我们从所有layers合成connections时，该结构变得与NSW graph相似**（在该case中，可以被放置的l相当于在NSW中的node degree）。对比起NSW，HNSW构建算法不需要在插入前进行shuffle——可以使用level randomization来达到随机化（stochasticity），从而真正允许增量索引（incremental indexing），即使数据分布随时间变化（。。。）。
