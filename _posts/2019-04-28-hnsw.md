...
HNSW的思想与著名的**1D概率跳表结构**非常相似，可以使用它的terms进行描述。与skip list的主要不同是：我们可以通过使用proximity graphs替代linked list来生成结构。HNSW方法可以使用相同的方式来做出分布式近似搜索／重叠结构（distributed approximate search/overlay structures）。
