...
CF的最流行方法是itemCF。它可以提供高质量的top-k结果。它的主要思想是，计算一个similarity matrix，相应的entries对应于pairwise item相似度（Person相关系数是一种流行选择）。为active users从头到尾计算推荐开销很大，因此，CF-based推荐通常会预计算好item-wise相似度，并以一种可快速检索的方式存储它们。根据该方法，预测active user ($$u_i$$)在item ($$v_j$$)上得分的任务包含了以下steps：

- 1.为$$u_i$$评分过的$$v_j$$寻找N个最相似的items，被称为$$v_j$$的N个最近邻。
