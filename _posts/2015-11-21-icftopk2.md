...
- 2.计算在N个items上的用户评分关于相似度相权的一个加权平均

一旦item scores被预测，那些带有最高预测得分的items会提供服务给用户。寻找top-k items的高效算法是项大挑战。在经典top-k settings中，对于m个features每个feature上的items得分可以从m个score-sorted lists中得到。该挑战可以通过TA/NRA算法族以及它的许多后代变种来解决。对比起经典的settings，在item-based CF中寻找top-k会有两个主要挑战：

- (1) 我们需要处理每个candidate item的聚合得分必须基于来自一个关于lists的不同set的entries来决定。这是因为每个candidate item的N个最相似items必须在一个给定user profile中不同
- (2) 加权平均并不是一个单调聚合函数（monotone aggregate function），它会在经典top-k算法中被合理利用. 在top-k算法中有一些工作是使用非单调聚合函数的[10,18,21]，据我们所知，在top-k问题上还没有工作是使用非单调聚合函数来解决lists的多个集合的组合。
...
- (ii) 中间层（Intermediate layer）: 包含了item-item matrix
- (iii) 应用层（Application layer）: 包含了高效运行topk算法

top-k算法可以独立于中间层（intermediate layer）
...
主要挑战是：在R中预测missing ratings。此后，我们使用$$r_{ij}$$来表示第i个user在第j个item上已存在评分（exsiting ratings)。我们将第i个user(row)表示为$$u_i$$、第j个item(column)表示成$$v_j$$。注意，从行为角度看，我们可以使用row $$u_i$$表示第i个user（列也如此）。我们使用$$r \in u_i$$来表示一个在user $$u_i$$的row上的已存在评分（existing rating）。我们使用$$\hat{r}_{ij}$$来做为active user $$u_i$$在一个candidate item $$v_j$$上的未知评分(unknown rating)的预测值。在该paper的其余部分，我们会使用$$\mu_i$$作为被$$u_i$$评分过的items数目，$$\mu$$表示被任意user评分过的items平均数目。
