...
$$y_{ui}$$为1表示user u和item i间有个交叉；然而，它并不意味着u实际会喜欢i。相似的，该值为0也并不表示用户u不喜欢item i。隐式数据中学习存在着许多挑战，因为它存在许多关于用户偏好的噪声数据。而已观察条目至少反映了用户在这些items上的兴趣，未观察条目可能只是缺失数据（missing data），它天然缺少负反馈。
