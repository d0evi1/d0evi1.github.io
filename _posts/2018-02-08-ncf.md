...
假设M和N表示users和items的数目。我们使用用户的隐式反馈来定义user-item交叉矩阵$$Y \in R^{M \times N}$$：
...
$$y_{ui}$$为1表示用户u和item i间有个交叉；然而，它并不意味着u实际会喜欢i。相似的，该值为0也并不表示用户u不喜欢item i。隐式数据中学习存在着许多挑战，因为它存在许多关于用户偏好的噪声数据。而观察到的条目至少反映了用户在这些items上的兴趣，未被观察到的条目可能只是缺失数据（missing data），它天然缺少负反馈。

隐式反馈的推荐问题，可以公式化成，估计在Y中未观察到条目的得分(score)，这些得分可以被用于进行ranking。Model-based方法假设，数据可以通过一个底层模型进行生成。正式的，他们可以被抽象成：学习$$\hat{y}_{ui}=f(u,i \mid \theta)$$，其中$$\hat{y}_{ui}$$可以表示交叉项$$y_{ui}$$的预测得分，$$\theta$$表示模型参数，f表示将模型参数映射到预测得分的函数（我们称之为“interaction function”）。

为了估计参数$$\theta$$，已经存在的机器学习方法会优化一个目标函数。有两种类型的目标函数最为常用——pointwise loss以及pairwise loss。作为在显式反馈上的一个天然扩展，在pointwise学习上的方法通常会根据一个回归框架，最小化在$$\hat{y}_{ui}与它的目标值$$y_{ui}$$间的平方loss（squared loss）。为了处理缺失的negative数据，会将所有未被观察到的条目当成是负反馈，或者从未观察到条目中进行抽样。对于pairwise学习，思想是，观察到条目应比未观察到条目排序更靠前。pairwise learning会最大化观察到条目$$\hat{y}_{ui}$$与未观察条目$$\hat{y}_{uj}$$间的间隔。

更近一步，我们的NCF框架会使用神经网络的方法对交叉函数f进行$$\hat{y}_{ui}$$的参数估计。这样，它能天然支持pointwise和pairwise的两种学习方法。

## 2.2 矩阵分解（MF）

MF会将分别将每个user和item使用一个关于隐特征的real-valued vector进行关联。假设$$p_u$$和$$q_i$$表示用户u和item i的隐向量；MF会将对$$y_{ui}$$的估计表示成$$p_u$$和$$q_i$$的内积：
...
其中，K表示隐空间的维度。MF会建模关于user和item隐因子的two-way interaction，假设隐空间的每个维度相互独立，并于进行使用相同的权重进行线性组合。这样，MF可以被看成是一个关于隐因子的线性模型。
...
为了允许CF的神经网络方法，我们采用multi-layer表示来建模一个user-item interaction $$y_{ui}$$，如图2所示，其中，一个layer的outout会看成是下一layer的input。底部的input layer包含了两个特征向量$$v_u^U$$和$$v_i^I$$，它们可以被定制化成支持更宽范围的users和items的建模，比如：context-aware和content-based，neighbor-based。由于本paper主要关注在纯粹的CF设置中，我们只使用了一个user和一个item的单位矩阵（identity）作为输入特征，并使用one-hot编码将它转成成一个关于输入的二值化稀疏特征表示。注意，这样的一个关于inputs的泛化特征表示，我们的方法可以进行简单的调整：通过使用content features来表示users和items来解决冷启动问题。
...
$$
L_{sqr} = \sum_{(u,i) \in y \union y^-} w_{ui}(y_{ui} - \hat{y}_{ui})^2
$$
...
$$
L = - \sum_{(u,i) \in Y} log (1-\hat{y}_{ui}) \\ = - \sum_{(u,i) \in Y \union Y^-} y_{ui} log \hat{y}_{ui} + (1-y_{ui}) log(1-\hat{y}_{ui})
$$
...
由于输入层（input layers）会使用user(item) ID的one-hot编码，获得的embedding vector可以看成是user(item)的隐向量。假设用户隐向量$$p_u$$是$$P^T v_u^U$$，而item隐向量$$q_i$$是$$Q^T v_i^I$$。我们定义第一个neural CF layer的映射函数是：
...
$$
\hat{y}_{ui} = \sigma(h^T a(p_u \odot q_i + W [] + b))
$$
...
$$
h \leftarrow [\alpha h^{GMF}, (1-\alpha)h^{MLP}]
$$
...(13)
