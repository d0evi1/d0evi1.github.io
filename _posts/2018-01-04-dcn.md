...
其中$$x_l, x_{l+1}$$是列向量（column vectors），分别表示来自第l层和第(l+1)层cross layers的输出：$$w_l, b_l \in R^d$$是第l层layer的weight和bias参数。在完成一个特征交叉f后，每个cross layer会将它的输入加回去，对应的mapping function f：$$R^d \rightarrow R^d $$，会满足残差$$x_{l+1} - x_l$$。一个cross layer的可视化如图2所示。
