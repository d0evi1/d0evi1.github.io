...
我们还通过为每个任务使用单个专家，研究了不同融合层级的配置。我们逐步增加融合层级的数量，并为每个层级使用单层MLP（多层感知机）。我们在不同融合层级下训练了四个模型，其MLP专家的隐藏维度分别为[256, 128]、[512, 256, 128]、[1024, 512, 256, 128] 和 [2048, 1024, 512, 256, 128]。对于任务塔，每个模型均采用隐藏维度为64的单层MLP。结果如表8所示。正如预期，**增加更多的融合层级会带来更大的性能提升**。即使将融合层级**增加到五层**，所有任务仍然都能观察到显著的改进。
