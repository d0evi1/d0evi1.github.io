...
为了分析echo chamber或filter bubble效应，**我们对于用户兴趣在什么时候发生非常剧烈变化非常感兴趣**。在我们的模型中，这会转成$$\mu_t(a)$$采用任意不同于初始兴趣$$\mu_0(a)$$的值：大的positive values表示用户会对item a变得非常感兴趣，而大的negative values表示用户不喜欢a。正式的，**对于一个有限item set M，我们会问到：L2-norm $$\| \mu_t - \mu_0 \|_2 = (\sum_{a \in M} (\mu_t(a) - \mu_0(a))^2)^{1/2}$$是否可以变得任意大？**如果满足下面条件，几乎必然用户的兴趣序列$$\mu_t$$被称为“弱退化（weakly  degenerate）”：
...
degenracy的一个更强概念是，它也需要：一旦$$\mu_t$$远离$$\mu_0$$：如果满足下面条件，几乎必须 序列$$\mu_t$$是一个较强的degenrate：
...
由于items通常以多样的类目呈现，我们简化它们间相互独立的猜想。通过对于所有t (例如：$$M = \lbrace a \rbrace $$)，设置l=1和$$a_t^1 = a$$，我们可以移除推荐系统的影响，并分开考虑用户的动态性( Dynamics)。这使得我们分析echo chamber effect：如果item a被无限次推到，兴趣 $$\mu_t(a)$$会发生什么？

由于a是固定的，为了简化概念，我们将$$\mu_t(a)$$替换成$$\mu_t$$。给定$$a_t$$，根据图1, $$\mu_{t+1}$$是一个关于$$\mu_t$$的函数（可能随机，由于$$\mu_{t+1}$$依赖于$$c_t$$和$$\mu_t$$，$$c_t$$依赖于$$\mu_t$$）。我们考虑当drift $$\mu_{t+1} - \mu_t$$是一个非线性随机函数时的通用case；对于drift的deterministic模型在附录B中。
