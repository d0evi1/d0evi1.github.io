...
嵌入层接收类别型（sparse）特征和dense特征的组合输入，输出$x_0 \in \mathbb{R}^d$。对于第$i$个类别型特征，我们通过$x_{\text{embed},i} = W_{\text{embed},i}\mathbf{e}_i$将其从高维稀疏空间投影到低维稠密空间，其中：

- $\mathbf{e}_i \in \{0,1\}^{v_i}$为one-hot向量
- $W \in \mathbb{R}^{e_i \times v_i}$为可学习投影矩阵  
- $x_{\text{embed},i} \in \mathbb{R}^{e_i}$为稠密嵌入向量
...
$$
x_{l+1} = x_0 \odot (W_lx_l + \mathbf{b}_l) + x_l \tag{1}
$$
其中：
- $x_0 \in \mathbb{R}^d$是包含原始一阶特征的基底层，通常设为嵌入层输入
- $x_l, x_{l+1} \in \mathbb{R}^d$分别表示第$(l+1)$层的输入和输出
...
我们探索两种组合结构：
1. **堆叠结构**（图1a）：输入$x_0$先经过交叉网络再输入深度网络，最终层$x_{\text{final}} = \mathbf{h}_{L_d}$, $\mathbf{h}_0 = x_{L_c}$，建模方式为$f_{\text{deep}} \circ f_{\text{cross}}$
2. **并行结构**（图1b）：输入$x_0$并行输入两个网络，最终拼接输出$x_{\text{final}} = [x_{L_c}; \mathbf{h}_{L_d}]$，建模方式为$f_{\text{cross}} + f_{\text{deep}}$

预测值$\hat{y}_i$计算为：

$$
\hat{y}_i = \sigma(\mathbf{w}_{\text{logit}}^\top x_{\text{final}})
$$
...
$$
x_{l+1} = x_0 \odot \left( U_l V_l^\top x_i + \mathbf{b}_l \right) + x_i \tag{2}
$$

其中$U_l, V_l \in \mathbb{R}^{d \times r}$且$r \ll d$。该式有两种解释：

- 1. 在子空间中进行特征交叉学习
- 2. 将输入$x$投影到低维空间$\mathbb{R}^r$后再重构回$\mathbb{R}^d$
...
E_i(x_l) = x_0 \odot \left( U_l^i V_l^{i\top} x_l + \mathbf{b}_l \right) \tag{3}
$$
...
$$
E_i(x_l) = x_0 \odot \left( U_l^i \cdot g\left( C_l^i \cdot g\left( V_l^{i\top} x_l \right) \right) + \mathbf{b}_l \right) \tag{4}
$$
...
设嵌入向量 $x = [x_1; x_2; \ldots; x_k] = [x_1, x_2, \ldots, x_d] \in \mathbb{R}^d$ 为列向量，其中 $x_i \in \mathbb{R}^{e_i}$ 表示第$i$个特征嵌入，$x_i$ 表示$x$中的第$i$个元素。定义多重指标 $\boldsymbol{\alpha} = [\alpha_1, \ldots, \alpha_d] \in \mathbb{N}^d$，且 $|\boldsymbol{\alpha}| = \sum_{i=1}^d \alpha_i$。
...
假设$l$层交叉网络的输入为$x \in \mathbb{R}^d$，输出为$f_l(x) = \mathbf{1}^\top x_l$，第$i$层定义为：
$$x_i = x \odot \mathbf{W}^{(i-1)}x_{i-1} + x_{i-1}$$
...
其中$x_{\text{pairs}} = [x_i \tilde{x}_j]_{\forall i,j}$包含$x_0$与$\tilde{x}$间所有$d^2$个成对交互；$\mathbf{w} \in \mathbb{R}^d$是DCN-V的权重向量，$\mathbf{w}_i \in \mathbb{R}^d$是DCN-V2权重矩阵的第$i$列(式(1))。
...
$$x_{1,h,*} = [\mathbf{I}, \mathbf{I}, \ldots, \mathbf{I}] (x \odot (\mathbf{W}x)) = \sum_{i=1}^k x_i \odot (\mathbf{W}_{i,:}x)$$
...
$$e_{xi} = \sum_{j=1}^k x_i \odot (\mathbf{W}_{i,j} x_j) = x_i \odot (\mathbf{W}_{i,:}x)$$
