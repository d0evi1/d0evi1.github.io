...
《Learning with Positive and Unlabeled Examples Using Weighted Logistic Regression》提出了一种PU learning的方法：

# 0.介绍

学习使用正例和未标记(unlabeled)样本的问题经常在检索应用中出现。我们将问题转化为带有噪声的学习问题，**通过将所有未标记样本标记为负例，并使用线性函数从噪声样本中学习**。为了学习带有噪声的线性函数，**我们在加权样本后进行逻辑回归，以处理噪声率大于一半的情况**。通过适当的正则化，逻辑回归问题的代价函数是凸的，允许问题有效解决。我们还提出了一种可以从正例和未标记样本中估计的性能度量，用于评估检索性能。该度量与精确度和召回率的乘积成比例，可以与验证集一起使用，选择逻辑回归的正则化参数。在文本分类语料库上的实验表明，所提出的方法有效。

# 1.引言 

在检索应用中，通常会出现这样的情况：可以获取正例和未标记样本，但**获取负例则需要支付额外的成本**。例如：

- 在尝试学习用户对网页的偏好分类器时，用户的书签可以被视为正例，而未标记样本可以从网页中采样。
- 在直接营销中，我们希望有一个分类器能够从客户档案中识别出未来的客户。公司当前的客户名单可以被视为正例，而与获取负例的成本相比，可以以较低的成本购买新的未标记样本数据库。

在本文中，我们使用以下简单的模型使用正例和未标记样本来进行学习：正例会以 $1-\alpha$的概率被随机标记为P（postive），其余则会以 $\alpha$标记为U（unlabeled状态）（见(Denis, 1998)）。在这种假设下，如果我们将所有unlabeled样本标记为负，我们将永远不会在负例上出错；但将以概率 $ \alpha $ 随机地将正例标记为负。

这种表述的一个问题是 $ \alpha $ 的值是未知的。Blum 和 Mitchell (Blum & Mitchell, 1998) 观察到，给定：
...
下面的表达式（假正例（FP）和假负例（FN）的错误率的期望求和）:

$$
Pr[f(X) = 1 | Y'=-1]+Pr[f(X) = -1 | Y'=1]
$$ 
...
由于目标函数具有零实际误差，因此**当目标函数在所使用的函数族中，并且样本量足够大时**，对观察到的假正例（FP）和假负例（FN）错误频率的和最小化，将给出目标函数的良好近似。这避免了需要知道 $ \alpha $ 的值。

最小化假正例（FP）和假负例（FN）的错误率，等价于：最小化预期加权误差（expected weighted error），其中假正例（FP）会乘以 $ Pr[Y'=-1] $，假负例（FN）乘以 $Pr[Y'=1]$。不幸的是，对于线性函数，最小化加权误差（weighted errors）是 NP-hard 的 (Hoffgen 等人，1995)。在本文中，我们不是最小化加权误差，而是通过执行逻辑回归来学习：在给定输入时观察到正标签的real-valued条件概率。我们通过构建cost函数来执行正则化，该函数从线性函数权重的平方和加权逻辑损失的总和中优化。最终的成本函数是凸的，我们使用简单的梯度下降进行优化。
