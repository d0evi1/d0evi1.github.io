...
为了解决这些限制，本文提出了一种名为Scenario-Adaptive Transformer（SATrans）的MS-CTR预测的显式特征交互模型，将场景信息纳入特征的相关建模中，以学习每个场景的独特和自适应的高阶特征交互。具体而言，我们利用Transformer [23]作为骨干架构，对输入特征进行高阶交互和组合建模，该方法已被AutoInt [21]和InterHAt [9]证明是有效的。Transformer中的多头自注意机制允许每个特征字段与所有其他特征交互，并自动识别相关特征以形成有意义的高阶特征。为了将场景特性纳入特征交叉中，我们首先设计了一个场景编码器，将场景特定特征转换为固定长度的场景embedding。然后利用场景自适应交互层来测量相关性，使用特征对的嵌入和场景嵌入作为输入，其中注意力分数通过一个精心设计的场景自适应相关函数计算。提出的场景自适应自注意机制赋予SATrans许多优点：（1）共性建模：每个交互层中的共享特征转换矩阵和嵌入层自然地捕捉到共同知识；（2）差异建模：自适应注意力分数编码了场景之间的分布偏移；（3）高可扩展性：网络参数的规模几乎不依赖于场景的数量，使SATrans能够高效地处理数千甚至数百万个场景；（4）良好的可解释性：注意力分数可以衡量特征之间的相关性，提供实例级和场景级的可解释性。总之，在本文中，我们做出了以下贡献：
