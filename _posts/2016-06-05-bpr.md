...
其中$$\hat{x}_{uij}(\Theta)$$是一个特定的关于模型参数向量$$\Theta$$的real-valued函数，它会捕获user u、item i、item j间的特殊关系。换句话说，我们的通用框架会将建模在u、i、j间的关系的任务表示到一个底层模型类（比如：MF或adaptive kNN）上，它们负责估计$$\hat{x}_{uij}(\Theta)$$。因而，统计方式建模一个个性化总顺序$$>_u$$变得可行。出于便利，后续章节我们会跳过来自$$\hat{x}_{xij}$$的参数$$\Theta$$。
