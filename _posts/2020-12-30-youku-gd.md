...
为了回答第1个问题，我们使用表1中的9个在线视频内容来测试pv-click-ctr模型的效果。我们选择[29]中提出的“smoothing CTR方法”作为baseline。这里的“smoothing CTR方法”使用一个empirical Bayes方法来处理层次化的天然数据。实验结果如图3所示。

在参数似合pv-click-ctr模型之前，会预处理历史天级PV和CLICK数据以及参数。
