...
我们将来自异构域的rank lists的集成(integration)定义为一个blending问题，将以如下方式将learning to blend rankings的问题进行公式化：
...
以这种方式进行Blending可以最大化Discounted Cumulative Gain(DCG)，可以为这些rankings保留排序（ordering）。
...
- 1) 对于一个items集合，我们只有items的ranking
- 2) 对于一个items集合，我们对于每个item都有一个score，items的ranking通过items的scores进行索引，例如：ranking是通过对items的scores进行sorting获得的

给定一个关于item sets和它相关rankings的pair，我们可以区分三种cases：

- 两个sets都在situation 1)。我们需要学习一个transformation：它可以将一个set中的ranks与另一set的ranks相关联
- 一个set在situation 1)中，另一个在situation 2)中。我们需要学习一个transformation：它可以将一个set中的ranks与另一个set中的scores相关联
...
对应于$$R_1^i$$和$$R_2^i$$，我们也具有共M+N items的combined ranking（出于简洁，这里假设两个list间没有重叠）：
...
相应的，我们定义了$$\lbrace (m,n) 1 \geq m \geq M, m \geq n \geq N \rbrace $$两个子集：$$S_i^+$$对应于$$d_{1m}^i$$的rank高于$$d_{2n}^i$$的cases，$$S_i^-$$对应于$$d_{1m}^i$$的rank低于$$d_{2n}^i$$的cases，并定义了：
...
其中，K是来自$$S_i^+$$和$$S_i^-, i=1, \cdots, Q$$ 两者的items的总数目。
...
等式(1)演示了两个domains的思想。该算法可以轻易地扩展到blend超过两个的rankings。给定来自X domains的ranking lists，选择其中一个作为参照点，其余X-1的转换$$(\alpha_1, \beta_1), \cdots, (\alpha_{X-1}, \beta_{X-1})$$。该QP问题的constraints会涉及到来自两个domains的item sets的所有pairs，例如：该问题将变为：

$$
\underset{\alpha_v, \beta_v, \zeta_k}{min} \sum_{k=1}^{K} \zeta_k^2 + \lambda_{1,1} \alpha_1^2 + \lambda_{1,2} \beta_1^2 + \lfloor + \lambda_{X-1,1} \alpha_{X-1}^2 + \lambda_{X-1,2} \beta_{X-1}^2
$$
