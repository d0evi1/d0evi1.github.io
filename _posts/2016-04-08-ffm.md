...
其中λ是正则参数，在loss function，如果是线性模型，则：
...
$$
\phi_{Poly2}(w,x) = \sum_{j_1=1}^{n} \sum_{j_2=j_1+1}^{n} w_h(j_1,j_2) x_{j_1} x_{j_2}
$$
...(2)
...
其中\$f_1\$和\$f_2\$各表示\$j_1\$和\$j_2\$的fields。如果f是fields的数目，那么FFM的变量数目为nfk，计算公式(4)的复杂度为\$ O(\hat{n}^2 k) \$。在FFM中，由于每个隐向量只需要学到一个特定field的影响，通常：
...
$$
g_{j_1,f_2} \equiv \delta_{w_j_1,f_2}f(w)=\lambda \cdot w_{j_1,f_2} + k \cdot w_{j_2,f_1} x_{j_1} x_{j_2}
$$
