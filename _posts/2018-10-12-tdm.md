...
注意，提出的抽样方法与hierarchical softmax相当不同。对比在hierarchical softmax中使用的方法，它会让模型混淆最优和次优结果，我们为每个正节点的同层级随机选择负样本。这种方法让每一层的判别器是一个内部层级全局的(intra-level global)。每个层级的全局判别器可以更独立的做出精准决策，不需要依赖于上层决策的好坏。全局判别能力对于hierarchical推荐方法非常重要。它可以确保，即使模型做出坏的决策，低质量节点会漏进到上层中的候选集，通过该模型在下层也能选中那些相对更好的节点，而非非常差的节点。
