...
在我们[36]的工作中，我们使用一种kNN简单算法，它基于m个searches的序列，并返回它们的最佳结果。对于每个后续的search时，寻找不到最邻近的概率会指数递减，无需重构建（reconstruction）就可以增强（boosting）该sturcture的accuracy。
...
- 2) 之前被访问过的elements的列表visitedSet，会跨搜索序列共享，以便阻止无用的重复抽取。 
...
由于我们构建一个Delaunay graph的近似（approximation），在construction算法细节上有很大的自由度。主要目标是，最小化false global minima的概率，并能保持links数目尽可能小。可以使用一些基于metric space拓朴的方法。例如，[13]中提出了构建近似Delaunay graph，它可以对graph中的每个vertex的一个固定数目的edges，来最小化相应的Voronoi region的volume（通过monte-carlo法）。我们提出通过1-by-1的方式插入数据来生成该sturcture，并在每个step将它们相连接（使用在该sturcture中已经存在的f个closest objects）。我们的方法基于elements set（它们是Voronoi neighbors）和f个closest elements的交叉（intersection）。
