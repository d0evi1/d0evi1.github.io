...
S的结构被构建成一个navigable small word网络，通过一个graph G(V,E)来表示，其中，来自集合X的objects是唯一映射到set V的顶点(vertices)上。edge E的集合通过结构构建算法来决定。由于每个vertex是唯一映射到集合X的一个element上，我们会交替使用术语“vertex”、"element"和"object"。我们会使用术语“friends”来表示共享一个edge的vertices。vertices的列表会与vertex $$v_i$$共享一个公共edge，被称为vertex $$v_i$$的friend list。
