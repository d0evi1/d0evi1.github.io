...
由graph G(V,E)（其中，来自集合X的objects是唯一映射到set V的顶点(vertices)上）表示的一个navigable small word网络，会通过structure S来构建。edge E的集合通过结构构建算法（structure construction algorithm）来决定。
