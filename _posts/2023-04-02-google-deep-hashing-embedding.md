...
图1 一个基于one-hot的full embedding和深度哈希嵌入（DHE）的示意图，用于为200万个ID生成32维嵌入。维度数字来自我们的实验，以提供一个具体的例子。这两种模型达到了类似的AUC，而DHE的大小仅为满模型的1/4。DHE使用密集哈希编码来为每个特征值获得一个唯一标识符，并应用深度嵌入网络来生成特征嵌入。DHE不执行任何嵌入查找
