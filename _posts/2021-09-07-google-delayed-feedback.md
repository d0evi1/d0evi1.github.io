...
- 首先将标签拆分为不同延迟分桶的标签之和，每个分桶仅训练成熟标签；
- 其次采用温度计编码提升精度并降低推理成本；
...
延迟建模与截断反馈问题密切相关。早期研究（Elkan & Noto, 2008等）虽具参考价值但无法直接适用。另有多篇论文（Agarwal等, 2010; Menon等, 2011; Lee等, 2012; Rosales等, 2012）研究转化优化，但未解决标签延迟反馈问题。  
...
解决方案：  
引入**温度计编码**——将标签分解为重叠分桶$[t_p, t_p + M), [t_p + d_1, t_p + M),...,[t_p + d_n, t_p + M)$，使得每个子模型的标签定义为从分桶起点到总窗口结束期间的事件总数。该方法优势：  
