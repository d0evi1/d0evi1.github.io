...
基于内积的神经网络（IPNN）中，我们首先定义了pair-wise特征交叉作为向量内积： $$g(f_i, f_j) = <f_i, f_j>$$。
...
$$
W_p^n \odot p = \sum_{i=1}^N \sum_{j=1}^N \theta_i^n \theta_j^n <f_i, f_j> = <\sum_{i=1}^N \delta_{i}^n, \sum_{i=1}^N \delta_i^n>
$$
...
$$
W_p^n \odot p = \sum_{i=1}^N \sum_{j=1}^N <\delta_n^i, \delta_n^j> <f_i, f_j> 
$$
