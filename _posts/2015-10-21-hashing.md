...
我们提出了一种**层次化分区器（hierarchical partitioner）**，对于一个label scorer：f(x,y), 一个训练集：\$(x_i,y_i), i=\lbrace 1,...,m \rbrace \$，以及之前定义的label集合D，它尝试优化目标：precision@k。对于一个给定的训练样本\$(x_i,y_i)\$以及label scorer，我们定义了：
