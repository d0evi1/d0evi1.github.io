...
非负（non-negativity constraint）约束会应用在W上，若有的话，学到的W表示了在items间的正向关系。约束条件 diag(W)=0 也会被使用，以避免trivial solutions（例如：optimal W是一个identical matrix，以便一个item总能推荐它身身，以便最小化 $$\frac{1}{2} \| A - AW \|_F^2$$）。另外，约束 diag(W)=0 确保了$$a_{ij}$$不会被用于计算 $$\hat{a}_{ij}$$。
