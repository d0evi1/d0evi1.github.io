...
- W是一个n x n的sparse matrix的聚合系数（aggregation coefficients），第j列对应于等式(1)中的$$w_j$$，每一行$$\hat{a}_i^T(\hat{a}_i^T = a_i^T W)$$表示在所有items上对于用户$$u_i$$的推荐分。
...
其中，$$\| W \|_1 = \sum_{i=1}^n \sum_{j=1}^{n} \mid w_{ij} \mid $$是W的entry-wise l1-norm，并且$$\| \cdot \|_F$$是matrix Frobenius norm（弗罗贝尼乌斯范数）。在等式3中，AW是推荐得分的预估矩阵（例如：$$\hat{A}$$） 乘以 等式2的sparse linear model。第一项$$\frac{1}{2} \| A - AW \|_F^2$$ （例如：平方residual sum）用来衡量linear model是如何拟合训练数据的，$$\| W \|_F^2$$和$$\| W \|_1^2$$分别是$$l_F$$ norm和l1-norm正则项。常数$$\beta$$和$$\lambda$$是正则参数。参数越大，正则化越强。非负（non-negativity constraint）约束会应用在W上，若有的话，学到的W表示了在items间的正向关系。约束条件 diag(W)=0 也会被使用，以避免trivial solutions（例如：optimal W是一个identical matrix，以便一个item总能推荐它身身，以便最小化 $$\frac{1}{2} \| A - AW \|_F^2$$）。另外，约束 diag(W)=0 确保了$$a_{ij}$$不会被用于计算 $$\hat{a}_{ij}$$。
