...
我们对数据进行mean-center并进行rotate：假设$$\mu = \frac{1}{\eta} \sum_i y_i^*$$是在第一次化简后的均值，并且$$M \in R^{d+1 \times n}$$是一个使用$$\mu$$沿着它的列进行复制的矩阵。该中心数据矩阵的SVD为：

$$
(Y^* - M) = W \Sigma U^T
$$

其中，数据items出现在$$Y^*$$的列中。矩阵W是一个$$(d+1) \times (d+1)$$的矩阵。$$W=[w_1, \cdots, w_{d+1}]$$的每一列定义了一个正交单位长度的特征向量（eigenvector），因此，每个$$w_j$$定义了一个超平面，每个$$y_i^* - \mu$$被投影到它上面。矩阵W是一个旋转矩阵，它会将这些vectors对齐到它的主成分(principal components)上。我们定义了中心旋转（centered rotation）作为我们的第二个转换：
