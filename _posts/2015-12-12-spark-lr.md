...
- standardization：在对模型进行fit前，是否对训练特征进行标准化(归一化)。模型的系数将总是返回到原始的scale上，它对用户是透明的。注意：当不使用正则化时，使用/不使用standardization，模型都应收敛到相同的解（solution）上。在R的GLMNET包里，缺省行为也为true。缺省为true。
