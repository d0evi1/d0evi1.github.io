...
图1: Tree-based deep推荐模型 (a) 用户偏好预测模型。我们首先以层次化的方式在对应的layers上的节点上来抽象用户行为。接着，抽象用户行为和目标节点（target node）一起与其它特征（比如：user profile）被用于模型的输入。 (b) 树结构(tree hierarchy)。每个item首先会使用一个投影函数$$\pi(\cdot)$$来被分配给一个不同的leaf node。在leaf level上的红色节点(items)会被选中作为候选集
