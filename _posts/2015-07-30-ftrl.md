...
$$
w_{t+1} = argmin_{w} ( g_{1:t} \cdot w + \frac{1}{2} \sum_{s=1)^{t} \sigma_{s} \|w-w_s\|_{2}^{2} + \lambda_1  \|w\|_1 )
$$

其中我们定义了\$ \sigma_{s} \$表示learning-rate schedule，比如：\$\sigma_{1:t}=\frac{1}{\eta_t}\$。表面上，这些更新看起来很不同，但实际上，当我们采用\$ \lambda_1=0 \$，它们产生一个系数向量的相同序列。然而，FTRL-Proximal会使用\$ \lamba_1 > 0\$更新，在引入稀疏化上效果很好（详见试验结果）。
