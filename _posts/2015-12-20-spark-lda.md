...
其中，参数α是一个k维的vector，相应的components: αi > 0, 其中 Γ(x)为Gamma函数。Dirichlet是在simplex上的一个合适分布——它在指数族内，具有有限维的充分统计量（finite dimensional sufficient statistics），与multinomial分布共轭。在paper第5部分，这些属性将有助于LDA的inference和parameter estimation算法。
...
变分参数的最优值，可以通过对变分分布和真实后验概率<img src="http://www.forkosh.com/mathtex.cgi?p(\theta,z|W,\alpha,\beta)">间的KL散度进行最小化得到。该最小化可以通过一个迭代型的固定点方法完成。特别的，我们在附录A.3中展示的，通过计算KL散度的导数，将它们等于0, 可以得到以下更新等式的pair：
