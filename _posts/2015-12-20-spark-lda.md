...
- 3.对于每个文档（N个词汇wn）: 
	...
	- (b) 以主题zn为条件，使用多项分布条件概率（multinomial probability）：$P(w_n| z_n, \beta)$选中一个词wn

在该基础模型上，做了一些简化假设。

- 首先，Dirichlet分布的维度k(主题变量z的维度)，假设是已知并且确定的。
- 第二，词概率由一个k x V的矩阵β进行参数化，其中$\beta_{ij}=p(w^j=1|z^i=1)$, 目前当成是一个待估计的确定量。
...
<img src="http://www.forkosh.com/mathtex.cgi?\alpha \leftarrow \alpha - \rou_{t} \hat{\alpha}(\gamma_t)"> 

<img src="http://www.forkosh.com/mathtex.cgi?\eta \leftarrow \rou_{t} \hat{\eta}(\lambda)"> ......(9)
