...
如上所述，<img src="http://www.forkosh.com/mathtex.cgi?p(W|α,β)">的计算很困难。然后，变分推断提供给我们一个在log likelihood上的下界，我们可以分别根据α和β进行最大化。我们找到了近似经验贝叶斯估计，通过一种交替变分EM过程(alternating variational EM procedure)，来分别对变分参数γ 和 φ，最大化下界。接着，变分参数确定下来后，再分别根据模型参数α 和 β，最大化下界。
