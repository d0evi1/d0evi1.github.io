...
为此，blei等提出了一种online VB算法，这种算法基于online随机优化(online stochastic optimization)，在大数据集上，它比batch算法更快地产生好的参数估计。Online LDA可以方便地分析大量文档集合，不需要本地存储，或者收集文档，每个文档可以以流式到达，看完后抛弃。

以下的等式重新标号。

假设有K个主题。每个主题定义了一个在词汇表上的多项分布，假设这些主题从一个Dirichlet上抽取得到: βk ∼ Dirichlet(η)。给定主题，LDA后，LDA为每个文档d假设了以下的生成过程。

- 首先，从主题<img src="http://www.forkosh.com/mathtex.cgi?\theta_d ~ Dirichlet(\alpha)">上抽取一个分布；
- 接着，对于在文档中的每个词i，从主题权重<img src="http://www.forkosh.com/mathtex.cgi?z_{di}∼θ_d"> 上抽取一个主题索引<img src="http://www.forkosh.com/mathtex.cgi?z_{di} \in {1, . . . , K }">，从选中的主题上抽取观察词<img src="http://www.forkosh.com/mathtex.cgi?w_{di}">，<img src="http://www.forkosh.com/mathtex.cgi?w_{di} ~ \beta_{z_{di}}">。出于简洁性，我们假设在θ 和 β上具有对称先验，但该假设很容易放宽。

注意，如果我们在主词分配z上进行求和，接着，我们得到：<img src="http://www.forkosh.com/mathtex.cgi?p(w_{di}|\theta_d,\beta)=\sum_k\theta_{dk}\beta_{kw}">。这会导致LDA的"multinomial PCA"解释。我们可以将LDA看成是一个概率型的词频n的矩阵(其中ndw是词w出现在文档d中的次数)，分解成一个关于主题权重θ的矩阵， 以及一个主题字典β。我们的工作可以看成是在线矩阵分解技术的一个扩展，它会最优化squared error来得到更泛化的概率公式。

我们可以使用LDA分析一个关于文档的语料，通过检查关于主题β，主题比例θ，主题分配z在条件文档上的后验分布。

## Batch算法

在VB推断中，真实的后验由一个更简单的分布q(z,θ,β)来近似，它由一个自由参数集进行索引。这些参数用于优化：最大化置信下界(ELBO:Evidence Lower BOund)

<img src="http://www.forkosh.com/mathtex.cgi?logp(W|\alpha,\eta) \geq \mathcal{L} (W,\phi,\gamma,\lambda) \triangleq \mathds{E}_q[logp(W,z,\theta,\beta|\alpha,\eta)] - \mathds{E}_q[logq(z,\theta,\beta)]"> ...... (1)
...
<img src="http://www.forkosh.com/mathtex.cgi? \mathcal{L} (W,\phi,\gamma,\lambda) = \sum_{d} \{ \mathds{E}_q[log p(w_d | \theta_d,z_d,\beta)] + \mathds{E}_q [log p(z_d | \theta_d)] - \mathds{E} [logq(z_d)] + \mathds{E} [logp(\theta_d|\alpha)] - \mathds{E} [log q(\theta_d)] + ( \mathds{E} [logp(\beta |\eta)] - \mathds{E} [log q(\beta)])/D \}"> ......(3)
...
<img src="http://www.forkosh.com/mathtex.cgi? L = \sum_d \sum_w n_{dw} \sum_k \phi_{dwk}( \mathds{E}_q[log\theta_{dk}] + \mathds{E}[log\beta_{kw}] - log\phi_{dwk}) - log\Gamma(\sum_k\gamma_{dk}) + \sum_k(\alpha-\gamma_{dk})\mathds{E}[log\theta_{dk}] + log\Gamma(\gamma_{dk}) + (\sum_k -log\Gamma(\sum_w\lambda_{kw}) + \sum_w(\eta-\lambda_{kw})\mathds{E}[log\beta_{kw}] + log\Gamma(\lambda_{kw}))/D + log\Gamma(K\alpha) - K log\Gamma(\alpha) + (log\Gamma(\mathcal{V}\eta)-\mathcal{V}log\Gamma(\eta))/D \triangleq \sum_d \ell(n_d,\phi_d,\gamma_d,\lambda)"> ......(4)
...
<img src="http://www.forkosh.com/mathtex.cgi?\phi_{dwk} \propto exp \{ \mathds{E}_q[log\theta_{dk}] + \mathds{E}_q[log\beta_{kw}]\}">
...
<img src="http://www.forkosh.com/mathtex.cgi?\mathds{E}[log\theta_{dk}]=\Phi(\gamma_{dk})- \Phi(\sum_{i=1}^K \gamma_{di})">

<img src="http://www.forkosh.com/mathtex.cgi?\mathds{E}[log\beta_{kw}]=\Phi(\gamma_{kw})- \Phi(\sum_{i=1}^V \gamma_{ki})"> ......(6)
