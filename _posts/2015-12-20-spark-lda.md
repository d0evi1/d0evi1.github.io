...
- 一个**词(word)**是离散数据的基本单位，被定义成一个来自词汇表的item，由{1,...,V}进行索引。我们将词表示成one-hot格式。这样，使用上标来表示components，在词汇表中第v个词被表示成一个**V维的向量w**，其中<img src="http://www.forkosh.com/mathtex.cgi?w^v=1, w^u=0(u\neq{v})">。
...
- 3.对于每个文档（N个词汇wn）: 
	...
	- (b) 以主题zn为条件，使用多项分布条件概率（multinomial probability）：<img src="http://www.forkosh.com/mathtex.cgi?P(w_n| z_n, \beta)">选中一个词wn
...
其中<img src="http://www.forkosh.com/mathtex.cgi?p(z_n|\theta)">可以简单认为：θi对于唯一的i，有<img src="http://www.forkosh.com/mathtex.cgi?{z_{n}}^{i}=1">。在θ上积分(θ上连续)，并在z上进行求和（z上离散），我们可以得到一个关于**文档的边缘分布（marginal distribution）**：
...
<img src="http://www.forkosh.com/mathtex.cgi?p(W)=\prod_{n=1}^{N} p(w_n)">

### Mixture of unigrams

如果unigram模型和一个离散随机主题变量z混合，我们就得到了一个mixture of unigrams model。在该混合模型下，每个文档的生成：首先选中一个主题z，然后以条件多项概率<img src="http://www.forkosh.com/mathtex.cgi?p(w|z)">独立生成N个词。一个文档的概率为：

<img src="http://www.forkosh.com/mathtex.cgi?p(W)=\sum_{z} p(z) \prod_{n=1}^{N} p(w_n|z)">

当从一个语料估计时，词分布可以看成是：假设每个文档只有一个主题，在该假设下的主题表示。该假设对于大语料集来说是一个很受限的模型。

相反的，LDA允许文档展现多个主题。这会额外带来另一个参数形销：有k-1参数与mixture of unigrams中的p(z)有关，而LDA模型中有k个参数与<img src="http://www.forkosh.com/mathtex.cgi?p(\theta|\alpha)">有关。

### pLSI

pLSI是另一个广泛使用的文档模型(Hofmann 1999)。pLSI模型认为，一个文档d，词wn，对于一个未观察到的主题z来说是条件独立的：

<img src="http://www.forkosh.com/mathtex.cgi?p(d,w_n)=p(d)\sum_z p(w_n|z) p(z|d)">

pLSI模型尝试放宽maxture of unigrams模型中作出的简化猜想：每个文档只由一个主题生成。在某种意义上，它会捕获概率：一个文档可以包含多个主题，因为<img src="http://www.forkosh.com/mathtex.cgi?p(z|d)">可看成是对于一个特定文档d的主题权重的混合。然后，注意，d是一个dummy index，指向在训练集中文档列表。这样，d是一个多项分布随机变量，值尽可能与训练文档一样多，模型学到的主题混合 p(z &#124; d)只对应于被训练的文档。出于该原因，pLSI并不是一种定义良好的（well-defined）文档生成模型；天然不支持使用它来分配概率给一个之前未见过的文档。
...
图6展示了变分推断过程的总结，对于 γ 和 φn具有合适的起始点。从该伪代码我们可以知道：LDA的变分推断算法，每次迭代需要O((N+1)k)次操作。经验上，我们可以找到对于单个文档的迭代数，与文档中词的数目相近似。因而总的操作数与<img src="http://www.forkosh.com/mathtex.cgi?N^2k">相近。
...
如上所述，<img src="http://www.forkosh.com/mathtex.cgi?p(W|\alpha,\beta)">的计算很困难。然而，变分推断提供给我们一个在log likelihood上的下界，我们可以分别根据α和β进行最大化。我们找到了近似经验贝叶斯估计，通过一种交替变分EM过程(alternating variational EM procedure)，来分别对变分参数γ 和 φ，最大化下界。接着，变分参数确定下来后，再分别根据模型参数α 和 β，最大化下界。
...
<img src="http://www.forkosh.com/mathtex.cgi?logp(W|\alpha,\eta) \geq \mathcal{L} (W,\phi,\gamma,\lambda) \triangleq E_q[logp(W,z,\theta,\beta|\alpha,\eta)] - E_q[logq(z,\theta,\beta)]"> ...... (1)
...
<img src="http://www.forkosh.com/mathtex.cgi?q(z_{di}=k)=\phi_{dw_{di}k}">

<img src="http://www.forkosh.com/mathtex.cgi?q(\theta_d)=Dirichlet(\theta_d;\gamma_d)">

<img src="http://www.forkosh.com/mathtex.cgi?q(\beta_k)=Dirichlet(\beta_k;\lambda_k)"> ......(2)

每个词的主题分布z的后验，可以由φ参数化，在每个文档上的主题权重θ可以由γ进行参数化，在主题上的后验β可以由λ进行参数化。为便于记忆，我们将λ作为"主题(the topics)"。等式1分解为：

<img src="http://www.forkosh.com/mathtex.cgi? \mathcal{L} (W,\phi,\gamma,\lambda) = \sum_{d} \{ E_q[log p(w_d | \theta_d,z_d,\beta)] + E_q [log p(z_d | \theta_d)] - E [logq(z_d)] + E [logp(\theta_d|\alpha)] - E [log q(\theta_d)] + ( E [logp(\beta |\eta)] - E [log q(\beta)])/D \}"> ......(3)

注意，我们在对文档求和中引入了每个语料的项，可以通过文档D的数目进行划分。该步可以帮助我们对online VB算法求导。

接着，我们将上述期望展开成变分参数的函数。变分目标函数只依赖于n_dw, 词w在文档中的出现次数。当使用VB时，文档可以由词数（word counts）来进行归纳：
...
其中V是词汇表的size，D是是文档数目。<img src="http://www.forkosh.com/mathtex.cgi?\ell(n_d,\phi_d,\gamma_d,\lambda)"> 表示文档d对ELBO的贡献度。

L可以使用在变分参数φ, γ, λ 上的坐标上升法进行优化：

<img src="http://www.forkosh.com/mathtex.cgi?\phi_{dwk} \propto exp \{ E_q[log\theta_{dk}] + E_q[log\beta_{kw}]\}">

<img src="http://www.forkosh.com/mathtex.cgi?\gamma_{dk}=\alpha + \sum_{w}n_{dw}\phi_{dwk}">

<img src="http://www.forkosh.com/mathtex.cgi?\lambda_{kw}=\eta+\sum_d n_{dw} \phi_{dwk}"> ......(5)

在q分布下logθ和logβ的期望为：

<img src="http://www.forkosh.com/mathtex.cgi?E[log\theta_{dk}]=\Phi(\gamma_{dk})- \Phi(\sum_{i=1}^K \gamma_{di})">

<img src="http://www.forkosh.com/mathtex.cgi?E[log\beta_{kw}]=\Phi(\gamma_{kw})- \Phi(\sum_{i=1}^V \gamma_{ki})"> ......(6)
...
对于主题λ的一个好的设置是，ELBO L尽可能地高，在算法E-step中拟合每个文档的变分参数γ 和 φ之后。将γ(nd, λ)和φ(nd , λ)设置为由E-step产生的γd 和 φd。我们的目标是设置λ，使下面目标最大化：

<img src="http://www.forkosh.com/mathtex.cgi?\mathcal{L}(n,\lambda) \triangleq
 \sum_d \ell (n_d,\gamma(n_d,\lambda), \phi(n_d,\lambda), \lambda)"> ......(7)
...
第t个词频向量<img src="http://www.forkosh.com/mathtex.cgi?n_t">被观察看，保持λ固定，我们执行一个E-step来找到局部最优解<img src="http://www.forkosh.com/mathtex.cgi?\gamma_{t}">和<img src="http://www.forkosh.com/mathtex.cgi?\phi_{t}">。接着，我们计算<img src="http://www.forkosh.com/mathtex.cgi?\hat{\lambda}">，如果整个语料由单个文档<img src="http://www.forkosh.com/mathtex.cgi?n_t">重复D次组成，λ的设置是最优的(给定φt)。D是提供给算法的唯一文档数目，比如：一个语料的size。（在真实online的例子中：<img src="http://www.forkosh.com/mathtex.cgi?D->\infty">，对应于β的经验Bayes估计）。我们接着更新λ，使用一个闰前先前值和<img src="http://www.forkosh.com/mathtex.cgi?\hat{\lambda}">加权平均。<img src="http://www.forkosh.com/mathtex.cgi?\hat{\lambda}">的权重为：<img src="http://www.forkosh.com/mathtex.cgi?\rho \triangleq (\tau + t)^{-\kappa}">，其中<img src="http://www.forkosh.com/mathtex.cgi?\kappa \in (0.5,1]">控制着<img src="http://www.forkosh.com/mathtex.cgi?\hat{\lambda}">的旧值被遗忘的比率，τ0 ≥ 0会减缓算法的早期迭代。条件κ ∈ (0.5, 1]需要保证收敛。我们会在下一节展示，online LDA对应于在变分目标L上的一个随机自然梯度算法（stochastic natural gradient algorithm）。
...
**Mini-batches**: 在随机学习中的公共技术是在每次更新时考虑多个观察（observations）以降噪。在online LDA中，这意味着计算<img src="http://www.forkosh.com/mathtex.cgi?\hat{\lambda}">使用S>1的观察：

<img src="http://www.forkosh.com/mathtex.cgi?\hat_{kw}=\eta + \frac{D}{S}\sum_{S} n_{tsk} \phi_{tskw}">......(8)

其中<img src="http://www.forkosh.com/mathtex.cgi?\n_{ts}">是在mini-batch t中的第s个文档。该文档的变分参数<img src="http://www.forkosh.com/mathtex.cgi?\phi_{ts}">和<img src="http://www.forkosh.com/mathtex.cgi?\gamma_{ts}">是使用普通E-step进行拟合。注意，当S=D 和 κ = 0时，online VB就成了batch VB。

**超参数估计**：在batch 变分LDA中，超参数α 和 η的点估计，可以拟合给出的γ 和 λ，它使用线性时间的Newton-Raphson法。我们同样将α 和 η的更新并入到online LDA:

<img src="http://www.forkosh.com/mathtex.cgi?\alpha \leftarrow \alpha - \rou_{t} \textasciitilde{\alpha}(\gamma_t)"> 

<img src="http://www.forkosh.com/mathtex.cgi?\eta \leftarrow \rou_{t} \textasciitilde{\eta}(\lambda)"> ......(9)

其中，<img src="http://www.forkosh.com/mathtex.cgi?\textasciitilde{\alpha}(\gamma_t)">是Hessian逆矩阵乘以梯度<img src="http://www.forkosh.com/mathtex.cgi?\triangledown_{\alpha}l(n_t,\gamma_t,\phi_t,\lambda">，<img src="http://www.forkosh.com/mathtex.cgi?\textasciitilde{\eta}(\lambda)">是Hessian的逆矩阵乘以梯度<img src="http://www.forkosh.com/mathtex.cgi?\triangledown_{\eta}\mathcal{L}">，<img src="http://www.forkosh.com/mathtex.cgi?\rou_{t} \triangleq (\tau_0 + t)^{-\kappa}">。
