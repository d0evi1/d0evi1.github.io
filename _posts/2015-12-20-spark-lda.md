...
对于主题模型，后验概率是很难计算的，可以通过近似后验推断来计算。当前的近似后验推送算法分为两大类：抽样方法（sampling approaches）和优化解法(optimization approaches)。抽样方法有MCMC，优化解法有VB（Variational Bayes）。VB在经验上比MCMC更快，与MCMC准确率相当，对于大数据集来说，VB是更吸引人。
...
为此，blei等提出了一种online VB算法，这种算法基于online随机优化(online stochastic optimization)，在大数据集上，它产生好的参数估计比batch算法更快。Online LDA可以方便地分析大量文档集合，不需要本地存储，或者收集文档，每个文档可以以流式到达，看完后抛弃。

以下的等式重新标号。

## Batch算法

在VB推断中，真实的后验由一个更简单的分布q(z,θ,β)来近似，由于一个自由参数集进行索引。这些参数用于优化：最大化置信下界(ELBO:Evidence Lower BOund)
...
最大化ELBO，相当于最小化q(z,θ,β)与后验<img src="http://www.forkosh.com/mathtex.cgi?p(z,\theta,\beta|W,\alpha,\eta)间的KL散度。我们选择一个完整的因子分布q:
...
<img src="http://www.forkosh.com/mathtex.cgi? \mathcal{L}(W,\phi,\gamma,\lambda)=\sum_{d} \{ \mathds{E}_q[logp(w_d|\theta_d,z_d,\beta)] + \mathds{E}_q[logp(z_d|\theta_d)] - \mathds{E}[logq(z_d)] + \mathds{E}[logp(\theta_d|\alpha] - \mathds{E}[logq(\theta_d)] + (\mathds{E}[logp(\beta|\eta)] - \mathds{E}[logq(\beta)])/D \}"> ......(3)
