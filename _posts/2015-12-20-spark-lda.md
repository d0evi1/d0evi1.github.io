...
- 一个**词(word)**是离散数据是基本单位，被定义成一个来自词汇表的item，由{1,...,V}进行索引。我们将词表示成one-hot格式。这样，使用上标来表示components，在词汇表中第v个词被表示成一个V维的向量w，其中<img src="http://www.forkosh.com/mathtex.cgi?w^v=1, w^u=0(u\neq{v})">。
