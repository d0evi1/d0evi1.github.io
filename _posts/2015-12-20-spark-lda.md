...
其中<img src="http://www.forkosh.com/mathtex.cgi?p(z_n|\theta)">可以简单认为：θi对于唯一的i，有<img src="http://www.forkosh.com/mathtex.cgi?{z_{n}}^{i}=1">。在θ上积分(连续)，并在z上进行求和（离散），我们可以得到一个**文档的边缘分布（marginal distribution）**：
