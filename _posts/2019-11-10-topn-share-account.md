...
第二，**generality problem**。当推荐项与共享账号下所有用户都有些相关时，会引发“generality problem”，但不会出现它们的任意一个，当多个用户的不同品味兴趣被合到同一个账户上时，推荐系统会更可能推荐被多数人喜欢的**通用items（general items）**，忽略掉单独的品味。
