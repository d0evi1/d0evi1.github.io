...
一般的推荐系统假设：每个用户账号(user account)表示单个用户(user)。然而，多个用户经常共享单个账号。一个示例是：一个家庭中的所有人会共享同一个在线视频账号、同一个在线音乐流账号、同一个在线电商账号、同一个商店的购物卡。

当多个用户共享账号时会引出三个问题。

首先，**dominance problem**。当所有推荐都只与某些共享账号的部分用户有关，至少一个用户不会获得任意相关推荐。只有少量用户会主宰该账号。另外，考虑对一个经常购物的家庭。他们通常会在购买家庭用品时偶尔也会为孩子购买玩具。现在，很有可能所有推荐会基于更多的家庭用品进行推荐，推荐系统对孩子来说是没啥用的。

第二，**generality problem**。当推荐只与在共享账号下所有用户的少量相关时，会引发通用性问题，但不会出现它们的任意一个，当多个用户的不同品味兴趣被合到同一个账户上时，推荐系统会更可能推荐被多数人喜欢的**通用items（general items）**，忽略掉单独的品味。
