...
- **$$\gamma$$**: 我们假设，stepsize $$\gamma$$是一个固定的常数。尽管这些处理器并不知道任何其它处理器是否已经修改了x，我们定义$$x_j$$是决策变量x在j执行更新后的状态(state)。由于两个处理器可能会在同时写入x，我们需要注意该定义，但我们会通过简单地随机方式断绝关系。注意，$$x_j$$通常使用一个旧的梯度进行更新，它基于x的一个值，更早时多个时钟周期读取（待修改）。我们使用$$x_{k(j)}$$来表示用于计算产生状态$$s_j$$的梯度或次梯度的决策变量值。
