...
在本paper中，我们提出了一种称为"HOGWILD!"的简单策略来消除与锁相关的开销：**无锁方式并行运行SGD**。在HOGWILD中，各处理器被允许公平访问共享内存，并且能随意更新内存私有变量（individual components）。如果处理器可以overwrite其它的处理器，这样的lock-free scheme注定失败。然而，当数据处理是稀疏时，这意味着单独的SGD steps只会修改决策变量的一小部分，我们展示了内存覆盖写（memory overwrites）很少发生，并且当它们发生时会引入很少的错误到计算中。我们展示了，理论上和实验上一个近似的与处理器数成线性加速，通常发生在稀疏学习问题上。
