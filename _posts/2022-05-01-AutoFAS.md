...
其中，$$F_k$$表示了在$$F_k, k=1, 2$$的features数目, $$\beta, \gamma$$影响着底层系统的不同并发数，可以由经验决定。

我们将这种expected feature latency饮食到常规loss function中，乘以一个scaling因子$$\lambda$$，它会控制着在accuracy和latency间的tradeoff。对于feature selection的最终的loss function为：
