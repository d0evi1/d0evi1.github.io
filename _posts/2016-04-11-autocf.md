...
对于激活函数 $$f(\cdot), g(\cdot)$$。这里，$$\theta = \lbrace W, V, \mu, b \rbrace$$对于转移 $$W \in R^{d \times k}, V \in R^{k \times d}$$，其中biases为： $$\mu \in R^k, b \in R^d $$。该目标函数对应于一个自关联的神经网络（auto-associative
