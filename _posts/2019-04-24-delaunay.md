...
$$
V(p_i) = \lbrace x | |p_i - x| \leq |p_j -x| \forall 1 \leq j \leq n \rbrace
$$
...
- 如果$$p_j$$是$$p_i$$的最近邻，那边$$\bar{p_i p_j}$$是一条Delaunay edge
- 存在这样一个圆，它穿过$$p_i$$和$$p_j$$，但不包含任意其它点 $$\Leftrightarrow$$ $$\bar{p_i p_j}$$是一条Delaunay edge
- $$p_i, p_j, p_k$$的外切圆为空 $$\Leftrightarrow$$ $$\triangle p_i p_j p_k是Delaunay三角形
