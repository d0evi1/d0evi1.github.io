...
这样，对于α = 0, 我们具有一个标准的一阶HMM。为了限制对于产生的模型的Bayes error rate的size，条件概率表Pα被限制为稀疏的。特别的，对于每个y,y'，<img src="http://www.forkosh.com/mathtex.cgi?p_\alpha(\cdot|y,y')">具有至多两个非零条目。对于每个y,x'，<img src="http://www.forkosh.com/mathtex.cgi?p_\alpha(\cdot|y,x')">可以至多三个非零条目。对于每个随机生成的模型，会生成1000个长度为25序列的样本，用于训练和测试。
