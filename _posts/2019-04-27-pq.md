...
它通过tuple $$(q_c(y), q_p(r(y)))$$表示。通过与一个value的二进制表示进行类比发，coarse quantizer提供了最多的有效位（significant bits），而product quantizer的code相对应于最小的有效位（significant bits）。

d(x,y)的estimator，其中x是query，y是database vector，可以通过x和$$\ddot{y}$$间的距离 \\ddot{y}(x,y)对比：
