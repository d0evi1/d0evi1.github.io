...
其中，squared distances $$d(u_j(x), c_{j,i})^2: j=1 \cdots m, i= 1 \cdots k^*$$，是在search之前计算好的。
...
使用PQ的最近邻搜索很快（对于每个距离计算，只需要m个加法），并且可以极大减小内存需求。另外，该search是穷举（exhaustive）的。该方法在global image description的内容上仍然是可扩展的。然而，如果每张图片通过一个local descriptors集合描述，穷举搜索是禁止的，因为我们需要索引数十亿descriptors并执行多个queries。
...
与“Video-Google”[26]方法相似，一个codebook使用k-means学到，来生成一个quantizer $$q_c$$，被称为“coarse quantizer”。对于SIFT descriptors，与$$q_c$$相关的centroids数目为$$k'$$，通常范围为$$k'=1000 ~ 1,000,000$$。对比在第3节中使用的product quantizers的k来说较小。
...
$$
''y \triangleq q_c(y) + q_p (y - q_c(y))
$$

...(29)

它通过tuple $$(q_c(y), q_p(r(y)))$$表示。通过与一个value的二进制表示进行类比，coarse quantizer提供了最多的有效位（significant bits），而product quantizer的code对应于最小的有效位（significant bits）。

d(x,y)的estimator，其中x是query，y是database vector，可以通过x和''y间的距离 ''y(x,y)对比：

$$
''d(x,y) = d(x,''y) = d(x-q_c(y), q_p(y-q_c(y)))
$$
...
$$
''d(x,y)^2 = \sum\limits_j d(u_j(x - q_c(y)), q_{p_j}(u_j(y-q_c(y))))^2
$$

...(31)

与ADC strategy相似，对于每个subquantizer $$q_{p_j}$$，在partial residual vector $$u_j(x-q_c(y))$$与$$q_{p_j}$$所有centroids $$c_{j,i}$$间的距离是预先计算好并进行存储好的。
