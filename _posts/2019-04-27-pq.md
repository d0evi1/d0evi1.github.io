...
构成$$R^D$$一部分的一个quantizer的k个cells。通过定义可知：**在同一cell $$V_i$$上的所有vectors，通过相同centroid $$c_i$$被构建**。一个quantizer的quality通常通过input vector x和它的reproduction value $$q(x)$$间的MSE来进行测量：
...
首先，**一个vector x必须被quantized到它最近的codebook centroid**，根据欧氏距离：
...
第二个Lloyd condition是重构值(reconstruction value)必须是在Voronoi cell上vectors的期望值：
