...
这样，有了一个关于$$\pi$$的完美predictor，**estimator $$\hat{V}_{\hat{\pi}}^h$$的期望值（expected value）是一个在policy h的真实值（true value）（即$$V^h$$）上有保证的下界**。然而，正如该断言的左侧建议，它可以是一个非常松的界，特别是当由h选中的action，被$$\pi$$选中概率通常较小时。
