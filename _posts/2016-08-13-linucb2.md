...
引理3.1展示了，一个policy h的估计$$\hat{V}_{\pi}^h$$的期望值是，关于policy h的true value的一个下界的近似，其中该近似(approximation)主要归因于在estimate $$\hat{\pi}$$中的errors，下界(lower bound)主要归因于阀值$$\tau$$。当$$\hat{\pi} = \pi$$时，引理3.1可以简化成：

$$
E_x [ I(\pi(h(x) | x) \geq \tau) \cdot (V^h(x)] \leq E[\hat{V}_{\hat{\pi}}^h] \leq V^h 
$$

这样，有了一个关于$$\pi$$的完美predictor，$$\hat{V}_{\hat{\pi}}^h$$是一个在policy h的true value上有保证的下界。然而，正如该断言的左侧建议，它可以是一个非常松的界，特别是当由h选中的action，被$$\pi$$选中概率通常较小时。

引理3.1中$$1/\tau$$的依赖有点烦，但不可避免。考虑到bandit问题的一个实例：它具有一个input x，两个action $$a_1,a_2$$。假设：对于一些positive $$\epsilon$$有$$\pi(a_1 \mid x) = \tau + \epsilon$$，当评估该policy时有$$h(x)=a_1$$。进一步假设：rewards总是为1 ,并且$$\hat{\pi}(a_1 \mid x) = \tau$$。那么，该estimator满足$$E[\hat{V}_{\hat{\pi}}^h]=\pi(a_1 \mid x) / \hat{\pi}(a_1 \mid x) = (\tau + \epsilon) / \tau $$。因而，在estimate中的期望值是：$$E[\hat{V}_{\hat{\pi}}^h] - V^h = \mid (\tau+\epsilon)/\tau - 1 \mid = \epsilon/tau $$，其中：$$\hat{\pi}$$的regret为：$$(\pi(a_1 \mid x) - \hat{\pi}(a_1 \mid x))^2 = \epsilon^2$$。
