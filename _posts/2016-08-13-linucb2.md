...
E_x \Biggl[ I(\pi(h(x) | x) \geq \tau) \cdot (V^h(x) - \frac{\sqrt{reg(x)}}{\tau}) \Biggr] & \leq E[\hat{V}_{\hat{\pi}}^h] \\
& \leq V^h + E_x \Bigg\[ I(\pi(h(x) \mid x) \geq \tau) \cdot \frac{\sqrt{reg(x)}}{\tau} \Biggr]
\end{cases}
$$
