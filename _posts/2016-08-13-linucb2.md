...
- 5.Propensity Scores：略

而我们的方法分为三个step:

- 1.对于每个event $$(x,a,r_a)$$，使用回归(regression)来估计logging policy会选择action a的概率(probability) 为**$$\hat{\pi}(a \mid x)$$**。这里，“probability”是随时间移动的——我们可以想像：在不同时间点，采用一个均匀随机的方式从policies集合（可能是deterministic）中抽取。
...
**定理3.2**：假设$$\hat{\pi}$$是任意从X到actions A分布的函数。假设H是任意deterministic policies的集合。定义：$$\hat{H}=\lbrace h \in H \mid \pi(h(x) \mid x) > \tau, \forall x \in X \rbrace$$以及$$\hat{h} = argmax_{h \in \hat{H}} \lbrace V^h \rbrace$$。
