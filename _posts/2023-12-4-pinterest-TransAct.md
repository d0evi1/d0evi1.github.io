...
- 我们使用嵌入层将类别型特征投影到dense特征，并在数值型特征上执行batch归一化
...
用户行为序列中的所有行为都是Pin级别的行为。对于每个行为，我们使用三个主要特征：行为的时间戳、行为类型和Pin的32维PinSage嵌入[38]。**PinSage是一个紧凑的embedding，编码了Pin的内容信息**。
...
如前所述，用户行为序列中的Pin内容由PinSage嵌入[38]表示。因此，用户行为序列中所有Pin的内容是一个矩阵 $ W_{\text{pins}} \in \mathbb{R}^{\mid S \mid \times d_{\text{PinSage}}} $。最终编码的用户行为序列特征是CONCAT($ W_{\text{actions}} $, $ W_{\text{pins}} $) $ \in \mathbb{R}^{\mid S \mid \times (d_{\text{PinSage}} + d_{\text{action}})} $。
