...
Univariate特征选择，通过单变量统计检验(univariate statistical tests)来选取最佳参数。它可以看成是estimator的预处理阶段。Scikit-learn封装了一些特征选择方法对象，实现了transform方法来完成：
...
- [SelectPercentile](http://scikit-learn.org/stable/modules/generated/sklearn.feature_selection.SelectPercentile.html#sklearn.feature_selection.SelectPercentile)：留下百分之多少的top高分features
- 为每个feature都使用常用的单变最统计检验（univariate statistical tests）：[SelectFpr](http://scikit-learn.org/stable/modules/generated/sklearn.feature_selection.SelectFpr.html#sklearn.feature_selection.SelectFpr)（FPR: false positive rate，假阳，即负正本判为正），[SelectFdr](http://scikit-learn.org/stable/modules/generated/sklearn.feature_selection.SelectFdr.html#sklearn.feature_selection.SelectFdr)（FDR: false discovery rate，伪发现率），[SelectFwe](http://scikit-learn.org/stable/modules/generated/sklearn.feature_selection.SelectFwe.html#sklearn.feature_selection.SelectFwe)（FWER: family wise error，多重比较谬误）
...
使用L1正则化作为惩罚项的线性模型，具有sparse的解决方案：这些要估计的相关系数许多都为0.当目标为降低数据的维度时，以便使用另一个分类器，可以用feature_selection.SelectFromModel来选择非0的相关系数。特别的，sparse的estimators对于线性回归（linear_model.Lasso）很有用，对于分类（linear_model.LogisticRegression and svm.LinearSVC）很有用。
